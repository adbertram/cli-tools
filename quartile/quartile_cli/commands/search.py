"""Search commands for Quartile CLI."""
COMMAND_CREDENTIALS = {
    "query": [
        "browser_session"
    ],
...
app = typer.Typer(help="Search Quartile", no_args_is_help=True)
...
def search_query(
    ...
    """
    Search for items on Quartile.

    Example:
        quartile search query "my search term" --limit 20 --table
        quartile search query "keyword" --filter "price:lt:100"
        quartile search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        quartile search item abc123 --table
        quartile search item "https://example.com/item/abc123"
        quartile search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    """
    List items from Quartile.

    Example:
        quartile search list --limit 100 --table
        quartile search list --filter "status:active"
        quartile search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        quartile search wildcard "*test*"
        quartile search wildcard "prefix*"
        quartile search wildcard "*pending*" --fields "status,notes"
        quartile search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
