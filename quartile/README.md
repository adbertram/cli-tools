...
A command-line interface for [Quartile](https://www.quartile.com/) using browser automation. CLI interface for Quartile browser automation
...
After installation, the `quartile` command will be available in your terminal.
...
# Check login status
quartile auth status

# Search for items
quartile search query "search terms"

# Get item details
quartile search item ITEM_ID
```

## Commands

### Authentication (`quartile auth`)
...
quartile auth login --profile work

# Use named profile for status check
quartile auth status --profile work

# Set default profile via environment variable
export QUARTILE_DEFAULT_PROFILE=work
quartile auth status  # Uses 'work' profile

# Logout specific profile
quartile auth logout --profile work
...
### Profiles (`quartile auth profiles`)

```bash
# List all profiles
quartile auth profiles list

# Show default profile
quartile auth profiles get default

# Switch default profile
quartile auth profiles set-default PROFILE_NAME

# Create a new profile
quartile auth profiles create PROFILE_NAME
```

### Search (`quartile search`)
...
# Limit results
quartile search query "search terms" --limit 10

# Get item details
quartile search item ITEM_ID
quartile search item https://example.com/item/123

# List all items
quartile search list
```

### Cache (`quartile cache`)

```bash
# Show cache status for the active profile
quartile cache status

# Clear cached responses for the active profile
quartile cache clear
```
...
QUARTILE_USERNAME=your_username
QUARTILE_PASSWORD=your_password

# Base URL
QUARTILE_BASE_URL=https://www.quartile.com/

# Browser settings (true = invisible, false = visible browser)
QUARTILE_HEADLESS=true

# Authentication Configuration
QUARTILE_AUTH_COOKIE_NAMES=session.*,auth,token,sid  # Regex patterns for auth cookies
QUARTILE_AUTH_SELECTOR=                               # CSS selector indicating authenticated state
QUARTILE_AUTH_URL_PATTERN=                            # URL pattern indicating login page
QUARTILE_AUTH_TIMEOUT=60                              # Seconds to wait for login
QUARTILE_AUTH_POLL_INTERVAL=2                         # Seconds between auth checks
QUARTILE_DEFAULT_PROFILE=default                      # Default profile name
```
...
- **Headless mode**: Set `QUARTILE_HEADLESS=false` to see the browser (useful for debugging)
...
export QUARTILE_HEADLESS=false
quartile search query "test"
```
