...
A command-line interface for [Pipedrive](https://www.pipedrive.com/) using browser automation. CLI interface for Pipedrive browser automation
...
After installation, the `pipedrive` command will be available in your terminal.

## Quick Start

```bash
# Login to Pipedrive
pipedrive auth login

# Check login status
pipedrive auth status

# Search for items
pipedrive search query "search terms"

# Get item details
pipedrive search item ITEM_ID
```

## Commands

### Authentication (`pipedrive auth`)
...
pipedrive auth login --profile work

# Use named profile for status check
pipedrive auth status --profile work

# Set default profile via environment variable
export PIPEDRIVE_DEFAULT_PROFILE=work
pipedrive auth status  # Uses 'work' profile

# Logout specific profile
pipedrive auth logout --profile work
...
### Profiles (`pipedrive auth profiles`)

```bash
# List all profiles
pipedrive auth profiles list

# Show default profile
pipedrive auth profiles get default

# Switch default profile
pipedrive auth profiles set-default PROFILE_NAME

# Create a new profile
pipedrive auth profiles create PROFILE_NAME
```

### Search (`pipedrive search`)
...
# Limit results
pipedrive search query "search terms" --limit 10

# Get item details
pipedrive search item ITEM_ID
pipedrive search item https://example.com/item/123

# List all items
pipedrive search list
```

### Cache (`pipedrive cache`)

```bash
# Show cache status for the active profile
pipedrive cache status
...
PIPEDRIVE_USERNAME=your_username
PIPEDRIVE_PASSWORD=your_password

# Base URL
PIPEDRIVE_BASE_URL=https://www.pipedrive.com/

# Browser settings (true = invisible, false = visible browser)
PIPEDRIVE_HEADLESS=true

# Authentication Configuration
PIPEDRIVE_AUTH_COOKIE_NAMES=session.*,auth,token,sid  # Regex patterns for auth cookies
PIPEDRIVE_AUTH_SELECTOR=                               # CSS selector indicating authenticated state
PIPEDRIVE_AUTH_URL_PATTERN=                            # URL pattern indicating login page
PIPEDRIVE_AUTH_TIMEOUT=60                              # Seconds to wait for login
PIPEDRIVE_AUTH_POLL_INTERVAL=2                         # Seconds between auth checks
PIPEDRIVE_DEFAULT_PROFILE=default                      # Default profile name
```
...
- **Headless mode**: Set `PIPEDRIVE_HEADLESS=false` to see the browser (useful for debugging)
...
export PIPEDRIVE_HEADLESS=false
pipedrive search query "test"
```
