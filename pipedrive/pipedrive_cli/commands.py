"""Search commands for Pipedrive CLI."""
COMMAND_CREDENTIALS = {
    "query": [
        "browser_session"
    ],
...
app = typer.Typer(help="Search Pipedrive", no_args_is_help=True)
...
def search_query(
    ...
    Example:
        pipedrive search query "my search term" --limit 20 --table
        pipedrive search query "keyword" --filter "price:lt:100"
        pipedrive search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        pipedrive search item abc123 --table
        pipedrive search item "https://example.com/item/abc123"
        pipedrive search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    Example:
        pipedrive search list --limit 100 --table
        pipedrive search list --filter "status:active"
        pipedrive search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        pipedrive search wildcard "*test*"
        pipedrive search wildcard "prefix*"
        pipedrive search wildcard "*pending*" --fields "status,notes"
        pipedrive search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
