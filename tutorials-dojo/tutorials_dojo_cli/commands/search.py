"""Search commands for TutorialsDojo CLI."""
import fnmatch
import typer
...
app = typer.Typer(help="Search TutorialsDojo", no_args_is_help=True)
...
def search_query(
    ...
    """
    Search for items on TutorialsDojo.

    Example:
        tutorials-dojo search query "my search term" --limit 20 --table
        tutorials-dojo search query "keyword" --filter "price:lt:100"
        tutorials-dojo search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        tutorials-dojo search item abc123 --table
        tutorials-dojo search item "https://example.com/item/abc123"
        tutorials-dojo search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    """
    List items from TutorialsDojo.

    Example:
        tutorials-dojo search list --limit 100 --table
        tutorials-dojo search list --filter "status:active"
        tutorials-dojo search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        tutorials-dojo search wildcard "*test*"
        tutorials-dojo search wildcard "prefix*"
        tutorials-dojo search wildcard "*pending*" --fields "status,notes"
        tutorials-dojo search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
