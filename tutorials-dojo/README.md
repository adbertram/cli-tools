...
A command-line interface for [TutorialsDojo](https://portal.tutorialsdojo.com/faq-for-affiliate-marketing/) using browser automation. Browser automation for Tutorials Dojo affiliate portal workflows
...
After installation, the `tutorials-dojo` command will be available in your terminal.

## Quick Start

```bash
# Login to TutorialsDojo
tutorials-dojo auth login

# Check login status
tutorials-dojo auth status

# Search for items
tutorials-dojo search query "search terms"

# Get item details
tutorials-dojo search item ITEM_ID
```

## Commands

### Authentication (`tutorials-dojo auth`)
...
tutorials-dojo auth login --force

# Check authentication status
tutorials-dojo auth status

# Test authentication against live browser
tutorials-dojo auth test

# Clear stored session
tutorials-dojo auth logout
```
...
tutorials-dojo auth login --profile work

# Use named profile for status check
tutorials-dojo auth status --profile work

# Set default profile via environment variable
export TUTORIALS_DOJO_DEFAULT_PROFILE=work
tutorials-dojo auth status  # Uses 'work' profile

# Logout specific profile
tutorials-dojo auth logout --profile work
...
### Profiles (`tutorials-dojo auth profiles`)

```bash
# List all profiles
tutorials-dojo auth profiles list

# Show default profile
tutorials-dojo auth profiles get default

# Switch default profile
tutorials-dojo auth profiles set-default PROFILE_NAME

# Create a new profile
tutorials-dojo auth profiles create PROFILE_NAME
```

### Search (`tutorials-dojo search`)
...
# Limit results
tutorials-dojo search query "search terms" --limit 10

# Get item details
tutorials-dojo search item ITEM_ID
tutorials-dojo search item https://example.com/item/123

# List all items
tutorials-dojo search list
```

### Cache (`tutorials-dojo cache`)

```bash
# Show cached responses
tutorials-dojo cache list

# Clear all cached responses
tutorials-dojo cache clear
```
...
TUTORIALS_DOJO_USERNAME=your_username
TUTORIALS_DOJO_PASSWORD=your_password

# Base URL
TUTORIALS_DOJO_BASE_URL=https://portal.tutorialsdojo.com/faq-for-affiliate-marketing/

# Browser settings (true = invisible, false = visible browser)
TUTORIALS_DOJO_HEADLESS=true

# Authentication Configuration
TUTORIALS_DOJO_AUTH_COOKIE_NAMES=session.*,auth,token,sid  # Regex patterns for auth cookies
TUTORIALS_DOJO_AUTH_SELECTOR=                               # CSS selector indicating authenticated state
TUTORIALS_DOJO_AUTH_URL_PATTERN=                            # URL pattern indicating login page
TUTORIALS_DOJO_AUTH_TIMEOUT=60                              # Seconds to wait for login
TUTORIALS_DOJO_AUTH_POLL_INTERVAL=2                         # Seconds between auth checks
TUTORIALS_DOJO_DEFAULT_PROFILE=default                      # Default profile name
```
...
- **Headless mode**: Set `TUTORIALS_DOJO_HEADLESS=false` to see the browser (useful for debugging)
...
export TUTORIALS_DOJO_HEADLESS=false
tutorials-dojo search query "test"
```
