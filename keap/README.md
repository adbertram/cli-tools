...
A command-line interface for the [Keap API](https://developer.keap.com/docs/restv2/). CLI for Keap REST API workflows
...
After installation, the `keap` command will be available in your terminal.

## Quick Start

```bash
# Authenticate with Keap
keap auth login
...
# Login with API key
keap auth login
keap auth login --api-key YOUR_API_KEY
...
```bash
# List all profiles
keap auth profiles list

# Show default profile
keap auth profiles get default

# Switch default profile
keap auth profiles set-default PROFILE_NAME

# Create a new profile
keap auth profiles create PROFILE_NAME
```
...
```bash
keap items list --limit 2
```

### Table Output Example

```bash
keap items list --limit 5
```
...
KEAP_REFRESH_TOKEN=<refresh_token>
KEAP_TOKEN_EXPIRES_AT=<timestamp>

# Optional: API base URL
KEAP_BASE_URL=https://keap.com/
```
...
```bash
keap items list | jq '.items[].id'
```

### Export Items to JSON File

```bash
keap items list --limit 200 > items.json
```
