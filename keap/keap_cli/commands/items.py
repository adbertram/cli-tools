...
app = typer.Typer(help="Manage keap items", no_args_is_help=True)
...
def items_list(
    ...
    Examples:
        keap items list
        keap items list --table
        keap items list --limit 10
        keap items list --filter "status:active"
        keap items list --properties "id,name"
        keap items list --properties "id,user.name,status"
    """
    try:
        client = get_client()
...
def items_get(
    ...
    Examples:
        ...
        keap items get ITEM_ID --table
        keap items get ITEM_ID --properties "id,name"
        keap items get ITEM_ID --properties "user.name,user.email"
    """
    try:
        client = get_client()
...
def items_search(
    ...
    Examples:
        keap items search "*invoice*"
        keap items search "INV-2024*"
        keap items search "*pending*" --fields "status,notes"
        keap items search "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
