"""Keap API client with automatic token management and exponential retry."""
...
class KeapClient:
    """Client for interacting with Keap API with automatic token management and retry."""
    ...
    def __init__(
        ...
        """
        Initialize Keap client from configuration.
        ...
        if not self.config.has_credentials():
            ...
            raise ClientError(
                ...
                "Run 'keap auth login' to authenticate."
            )
    ...
    def _refresh_token(self):
        ...
        if not refresh_token:
            raise ClientError(
                "No refresh token available. Run 'keap auth login' to re-authenticate."
            )
        ...
        # self._update_headers()
        raise ClientError("Token refresh not implemented. Run 'keap auth login'.")
    ...
    def _make_request(
        ...
        """
        Make an HTTP request to the Keap API with exponential retry.
...
_client: Optional[KeapClient] = None


def get_client() -> KeapClient:
    """Get or create the global Keap client instance."""
    global _client
    if _client is None:
        _client = KeapClient()
    return _client
