...
from ..client import GoogleLighthouseClient, get_client
...
def print_records(records, table: bool, properties: Optional[str], single: bool = False) -> None:
    ...
    if table:
        rows = output if not single else [output]
...
def audits_list(
    ...
    try:
        client = get_client()
        audits = client.list_audits(limit=limit, filters=filter)
