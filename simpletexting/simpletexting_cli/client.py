"""Simpletexting API client with automatic token management and exponential retry."""
...
class SimpletextingClient:
    """Client for interacting with Simpletexting API with automatic token management and retry."""
    ...
    def __init__(
        ...
        """
        Initialize Simpletexting client from configuration.
        ...
        if not self.config.has_credentials():
            ...
            raise ClientError(
                ...
                "Run 'simpletexting auth login' to authenticate."
            )
    ...
    def _refresh_token(self):
        ...
        if not refresh_token:
            raise ClientError(
                "No refresh token available. Run 'simpletexting auth login' to re-authenticate."
            )
        ...
        # self._update_headers()
        raise ClientError("Token refresh not implemented. Run 'simpletexting auth login'.")
    ...
    def _make_request(
        ...
        """
        Make an HTTP request to the Simpletexting API with exponential retry.
...
_client: Optional[SimpletextingClient] = None


def get_client() -> SimpletextingClient:
    """Get or create the global Simpletexting client instance."""
    global _client
    if _client is None:
        _client = SimpletextingClient()
    return _client
