...
A command-line interface for [Devolutions](https://devolutions.net/buy/affiliates/) using browser automation. Browser automation CLI for Devolutions affiliate workflows
...
After installation, the `devolutions` command will be available in your terminal.

## Quick Start

```bash
# Login to Devolutions
devolutions auth login

# Check login status
devolutions auth status

# Search for items
devolutions search query "search terms"

# Get item details
devolutions search item ITEM_ID
```

## Commands

### Authentication (`devolutions auth`)
...
devolutions auth login --force

# Check authentication status
devolutions auth status

# Test authentication against live browser
devolutions auth test

# Clear stored session
devolutions auth logout
```
...
devolutions auth login --profile work

# Use named profile for status check
devolutions auth status --profile work

# Set default profile via environment variable
export DEVOLUTIONS_DEFAULT_PROFILE=work
devolutions auth status  # Uses 'work' profile

# Logout specific profile
devolutions auth logout --profile work
...
### Profiles (`devolutions auth profiles`)

```bash
# List all profiles
devolutions auth profiles list

# Show default profile
devolutions auth profiles get default

# Switch default profile
devolutions auth profiles set-default PROFILE_NAME

# Create a new profile
devolutions auth profiles create PROFILE_NAME
```

### Search (`devolutions search`)
...
# Limit results
devolutions search query "search terms" --limit 10

# Get item details
devolutions search item ITEM_ID
devolutions search item https://example.com/item/123

# List all items
devolutions search list
```

### Cache (`devolutions cache`)

```bash
# Show cached responses
devolutions cache list

# Clear all cached responses
devolutions cache clear
```
...
DEVOLUTIONS_USERNAME=your_username
DEVOLUTIONS_PASSWORD=your_password

# Base URL
DEVOLUTIONS_BASE_URL=https://devolutions.net/buy/affiliates/

# Browser settings (true = invisible, false = visible browser)
DEVOLUTIONS_HEADLESS=true

# Authentication Configuration
DEVOLUTIONS_AUTH_COOKIE_NAMES=session.*,auth,token,sid  # Regex patterns for auth cookies
DEVOLUTIONS_AUTH_SELECTOR=                               # CSS selector indicating authenticated state
DEVOLUTIONS_AUTH_URL_PATTERN=                            # URL pattern indicating login page
DEVOLUTIONS_AUTH_TIMEOUT=60                              # Seconds to wait for login
DEVOLUTIONS_AUTH_POLL_INTERVAL=2                         # Seconds between auth checks
DEVOLUTIONS_DEFAULT_PROFILE=default                      # Default profile name
```
...
- **Headless mode**: Set `DEVOLUTIONS_HEADLESS=false` to see the browser (useful for debugging)
...
export DEVOLUTIONS_HEADLESS=false
devolutions search query "test"
```
