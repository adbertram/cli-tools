...
def search_query(
    ...
    Example:
        devolutions search query "my search term" --limit 20 --table
        devolutions search query "keyword" --filter "price:lt:100"
        devolutions search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        devolutions search item abc123 --table
        devolutions search item "https://example.com/item/abc123"
        devolutions search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    Example:
        devolutions search list --limit 100 --table
        devolutions search list --filter "status:active"
        devolutions search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        devolutions search wildcard "*test*"
        devolutions search wildcard "prefix*"
        devolutions search wildcard "*pending*" --fields "status,notes"
        devolutions search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
