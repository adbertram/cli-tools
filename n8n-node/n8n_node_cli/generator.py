...
def _generate_credential_fields(metadata: CLIToolMetadata) -> str:
    ...
    for cred in metadata.credentials:
        # Use field_name computed by parser
...
def _generate_credential_env_code(metadata: CLIToolMetadata) -> str:
    """Generate TypeScript code that maps n8n credentials and config fields to env vars.

    Produces code that reads credentials from n8n and config from node parameters,
    then passes them as environment variables to the CLI subprocess.
    """
    ...
    if has_creds:
        lines.append(f"\t\tconst credentials = await this.getCredentials('{camel_name}Api');")
        for cred in metadata.credentials:
            lines.append(
                f"\t\tif (credentials.{cred.field_name}) env['{cred.env_var}'] = credentials.{cred.field_name} as string;"
            )
...
def _bundle_cli_source(cli_name: str, tools_dir: str, pkg_dir: Path):
    ...
    if common_src.is_dir():
        ...
        # Patch pyproject.toml: replace git+https:// reference to cli-tools-shared
        # with a local file reference so pip installs from the bundled copy
        pyproject_path = dst / "pyproject.toml"
        if pyproject_path.exists():
            content = pyproject_path.read_text()
            patched = re.sub(
                r'cli-tools-shared\s*@\s*git\+https://[^"]+',
                'cli-tools-shared @ file:./cli-tools-shared',
                content,
            )
            if patched != content:
...
def generate_node_package(
    ...
    # Generate credential env var setup code (reads n8n creds, passes as env vars to CLI)
    credential_env_setup = _generate_credential_env_code(metadata)
    # Generate config properties (top-level node params for non-auth env vars)
    config_properties = _generate_config_properties(metadata)
    # Generate config env code (reads config params, passes as env vars)
    config_env_code = _generate_config_env_code(metadata)
    # Pass env to execFileSync when credentials exist
    has_env = bool(metadata.credentials)
    exec_env_arg = ",\n\t\t\t\t\t\tenv" if has_env else ""
    ...
    if tools_dir:
        _bundle_cli_source(metadata.name, tools_dir, pkg_dir)
