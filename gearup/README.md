...
A command-line interface for [Gearup](https://www.gearupbooster.com) using browser automation. Browser CLI for GearUP workflows
...
After installation, the `gearup` command will be available in your terminal.
...
# Search for items
gearup search query "search terms"

# Get item details
gearup search item ITEM_ID
```

## Commands

### Authentication (`gearup auth`)
...
gearup auth login --profile work

# Use named profile for status check
gearup auth status --profile work

# Set default profile via environment variable
export GEARUP_DEFAULT_PROFILE=work
gearup auth status  # Uses 'work' profile

# Logout specific profile
gearup auth logout --profile work
...
### Profiles (`gearup auth profiles`)

```bash
# List all profiles
gearup auth profiles list

# Show default profile
gearup auth profiles get default

# Switch default profile
gearup auth profiles set-default PROFILE_NAME

# Create a new profile
gearup auth profiles create PROFILE_NAME
```
...
# Limit results
gearup search query "search terms" --limit 10

# Get item details
gearup search item ITEM_ID
gearup search item https://example.com/item/123
...
GEARUP_USERNAME=your_username
GEARUP_PASSWORD=your_password

# Base URL
GEARUP_BASE_URL=https://www.gearupbooster.com

# Browser settings (true = invisible, false = visible browser)
GEARUP_HEADLESS=true

# Authentication Configuration
GEARUP_AUTH_COOKIE_NAMES=session.*,auth,token,sid  # Regex patterns for auth cookies
GEARUP_AUTH_SELECTOR=                               # CSS selector indicating authenticated state
GEARUP_AUTH_URL_PATTERN=                            # URL pattern indicating login page
GEARUP_AUTH_TIMEOUT=60                              # Seconds to wait for login
GEARUP_AUTH_POLL_INTERVAL=2                         # Seconds between auth checks
GEARUP_DEFAULT_PROFILE=default                      # Default profile name
```
...
- **Headless mode**: Set `GEARUP_HEADLESS=false` to see the browser (useful for debugging)
...
export GEARUP_HEADLESS=false
gearup search query "test"
```
