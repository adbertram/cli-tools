"""Search commands for Gearup CLI."""
COMMAND_CREDENTIALS = {
...
app = typer.Typer(help="Search Gearup", no_args_is_help=True)
...
def search_query(
    ...
    """
    Search for items on Gearup.

    Example:
        gearup search query "my search term" --limit 20 --table
        gearup search query "keyword" --filter "price:lt:100"
        gearup search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        gearup search item abc123 --table
        gearup search item "https://example.com/item/abc123"
        gearup search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    """
    List items from Gearup.

    Example:
        gearup search list --limit 100 --table
        gearup search list --filter "status:active"
        gearup search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        gearup search wildcard "*test*"
        gearup search wildcard "prefix*"
        gearup search wildcard "*pending*" --fields "status,notes"
        gearup search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
