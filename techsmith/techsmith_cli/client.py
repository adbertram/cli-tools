"""Techsmith client using shared browser session tooling."""
...
class TechsmithClient:
    ...
    def search(
        ...
        """Search the TechSmith affiliate program record."""
    ...
    def get_item(self, item_id: str) -> ItemDetail:
        """Get details for the TechSmith affiliate program record."""
        for item in self.list_items(limit=1):
            if item.id == item_id:
                return create_item_detail(item.model_dump())
        raise ClientError(f"TechSmith item not found: {item_id}")
    ...
    def list_items(
        ...
        """List the TechSmith affiliate program record."""
...
_client: Optional[TechsmithClient] = None


def get_client() -> TechsmithClient:
    """Get or create the global Techsmith client instance."""
    global _client
    if _client is None:
        _client = TechsmithClient()
    return _client
