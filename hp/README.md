...
A command-line interface for [Hp](https://www.hp.com/us-en/shop/cv/affiliate-program) using browser automation. Browser automation CLI for HP affiliate workflows
...
After installation, the `hp` command will be available in your terminal.
...
# Search for items
hp search query "search terms"

# Get item details
hp search item ITEM_ID
```

## Commands

### Authentication (`hp auth`)
...
hp auth login --profile work

# Use named profile for status check
hp auth status --profile work

# Set default profile via environment variable
export HP_DEFAULT_PROFILE=work
hp auth status  # Uses 'work' profile

# Logout specific profile
hp auth logout --profile work
...
### Profiles (`hp auth profiles`)

```bash
# List all profiles
hp auth profiles list

# Show default profile
hp auth profiles get default

# Switch default profile
hp auth profiles set-default PROFILE_NAME

# Create a new profile
hp auth profiles create PROFILE_NAME
```
...
# Limit results
hp search query "search terms" --limit 10

# Get item details
hp search item ITEM_ID
hp search item https://example.com/item/123
...
HP_USERNAME=your_username
HP_PASSWORD=your_password

# Base URL
HP_BASE_URL=https://www.hp.com/us-en/shop/cv/affiliate-program

# Browser settings (true = invisible, false = visible browser)
HP_HEADLESS=true

# Authentication Configuration
HP_AUTH_COOKIE_NAMES=session.*,auth,token,sid  # Regex patterns for auth cookies
HP_AUTH_SELECTOR=                               # CSS selector indicating authenticated state
HP_AUTH_URL_PATTERN=                            # URL pattern indicating login page
HP_AUTH_TIMEOUT=60                              # Seconds to wait for login
HP_AUTH_POLL_INTERVAL=2                         # Seconds between auth checks
HP_DEFAULT_PROFILE=default                      # Default profile name
```
...
- **Headless mode**: Set `HP_HEADLESS=false` to see the browser (useful for debugging)
...
export HP_HEADLESS=false
hp search query "test"
```
