...
app = typer.Typer(help="Search Hp", no_args_is_help=True)
...
def search_query(
    ...
    """
    Search for items on Hp.

    Example:
        hp search query "my search term" --limit 20 --table
        hp search query "keyword" --filter "price:lt:100"
        hp search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        hp search item abc123 --table
        hp search item "https://example.com/item/abc123"
        hp search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    Example:
        hp search list --limit 100 --table
        hp search list --filter "status:active"
        hp search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        hp search wildcard "*test*"
        hp search wildcard "prefix*"
        hp search wildcard "*pending*" --fields "status,notes"
        hp search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
