...
class Item(Area):
    ...
    def field_value_v2(self, item_id, field_or_external_id):
        return self.transport.GET(url='/item/%s/value/%s/v2' % (item_id, field_or_external_id))

    def find_by_external_id(self, app_id, external_id):
        return self.transport.GET(url='/item/app/%d/external_id/%s' % (app_id, external_id))
    ...
    def export(self, app_id, exporter='xlsx', attributes=None):
        """
        Export items from an application to Excel format.

        Creates a batch for exporting items. Valid exporters are 'xls' and 'xlsx'.

        :param app_id: Application ID to export from
        :type app_id: int
        :param exporter: Export format ('xls' or 'xlsx')
        :type exporter: str
        :param attributes: Optional export parameters (filters, limit, offset, etc.)
        :type attributes: dict
        :return: Batch information with batch_id
        :rtype: dict
        """
        if attributes is None:
            attributes = {}
        if not isinstance(attributes, dict):
            raise TypeError('attributes must be of type dict')

        body = json.dumps(attributes)
        return self.transport.POST(
            url='/item/app/%d/export/%s' % (app_id, exporter),
            body=body,
            type='application/json'
        )
...
class Application(Area):
    ...
    def update(self, app_id, attributes):
        """
        Updates the app with the given id.

        :param app_id: Application ID
        :type app_id: str or int
        :param attributes: Configuration attributes to update (e.g., name, item_name, description, icon_id)
        :type attributes: dict
        :return: Python dict of JSON response
        :rtype: dict
        """
        if not isinstance(attributes, dict):
            raise TypeError('Must be of type dict')
        attributes = json.dumps(attributes)
        return self.transport.PUT(url='/app/%s' % app_id, body=attributes, type='application/json')
...
class Batch(Area):
    """Handles batch operations like export status checking."""
