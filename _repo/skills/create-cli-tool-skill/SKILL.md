...
```text
<cli-tools-root>/_repo/skills/<tool>-cli/   # SKILL.md + usage.json
```
</quick_start>
...
```
<cli-tools-root>/_repo/skills/<tool>-cli/
...
**Verification:** Run `scripts/validate-usage-json.sh <cli-tools-root>/_repo/skills/<tool>-cli/usage.json` against a CLI skill with a root-level leaf command and confirm the summary is `PASSED: 0 errors, 0 warning(s)`.
