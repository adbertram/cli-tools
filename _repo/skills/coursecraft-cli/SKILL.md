...
**Verification:** Run `uv run --project /Users/adam/Dropbox/GitRepos/cli-tools/_personal/coursecraft --with pytest python -m pytest /Users/adam/Dropbox/GitRepos/cli-tools/_personal/coursecraft/tests/test_demo_build_products_parse.py` and resync the affected demo XML file, then verify the record includes the `Requirements` field.
...
**Verification:** `uv run --project /Users/adam/Dropbox/GitRepos/cli-tools/_personal/coursecraft --with pytest python -m pytest /Users/adam/Dropbox/GitRepos/cli-tools/_personal/coursecraft/tests/test_clips_update.py -q` (includes `test_update_clip_accepts_only_brainstorming_outline_fact_checked`), then `uv run --project /Users/adam/Dropbox/GitRepos/cli-tools/_personal/coursecraft --with pytest python -m pytest`. Live: read `fields["Brainstorming Outline Fact Checked"]` with `coursecraft clips get <id>`, run `coursecraft clips update <id> --brainstorming-outline-fact-checked` with no other args, confirm no "No fields to update" error and that the read-back is true.
