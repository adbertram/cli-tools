...
| Check database access | `things auth status` |
</quick_start>
...
- **auth** -- Check database access status (login, logout, status)
...
**Fix:** Patched `<cli-tools-root>/things/things_cli/client.py`:
...
```bash
<cli-tools-root>/_repo/skills/cli-tool/scripts/install-cli-tool.sh things
```
...
**Fix:** Added `--project` and `--area` flags to `things todos update` (mutually exclusive). Editable install at `<cli-tools-root>/things/things_cli/`:
