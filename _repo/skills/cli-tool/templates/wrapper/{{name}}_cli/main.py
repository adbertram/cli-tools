"""Main entry point for {{Name}} CLI wrapper."""
...
def list_items(
    ...
    """List items from the underlying CLI."""
...
@command
def get_item(
    item_id: str = typer.Argument(..., help="Item ID or name"),
    table: bool = typer.Option(False, "--table", "-t", help="Display as table"),
    properties: Optional[str] = typer.Option(None, "--properties", "-p", help="Comma-separated fields to include"),
):
    """Get a single item from the underlying CLI."""
...
def search_items(
    ...
    """Search items from the underlying CLI output."""
