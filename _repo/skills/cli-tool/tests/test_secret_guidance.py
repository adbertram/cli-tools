...
def test_cli_tool_expert_agent_carries_secret_manager_rule():
    ...
    codex_text = _read_repo("_repo/agents/cli-tool-expert.toml")
    claude_text = _read_repo("_repo/agents/cli-tool-expert.md")
    assert expected in codex_text, (
        "cli-tool-expert.toml must require the secret manager instead of `.env` files."
    )
    assert expected in claude_text, (
