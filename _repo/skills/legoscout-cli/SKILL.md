...
| Create a new immutable server baseline and pull crops | `legoscout deploy pull-db --output <NEW_BASELINE>` |
| Deploy code without publishing data | `legoscout deploy push` |
| Verify availability on the authoritative server | `legoscout deploy expire` |
| Append a run learning on the authoritative server | `legoscout deploy source-note <SOURCE> --text <TEXT> [--date <YYYY-MM-DD>]` |
| Prepare validated observations against a baseline | `legoscout deploy prepare-ingest --run-id <ID> --baseline <BASELINE> --records <JSON_ARRAY> --output <NEW_PAYLOAD>` |
| Publish the prepared observations transactionally | `legoscout deploy ingest <PAYLOAD>` |
...
| Append a source note during local maintenance | `legoscout sources notes add <SOURCE> --text <TEXT>` |
...
- **deploy** -- Server ingestion and separate code deployment (subcommands: expire, ingest, prepare-ingest, pull-db, push, rollback, status)
