...
| `scripts/build-audit-output.sh` | The run is a report-only transaction audit (`--audit`) or recurring-expense audit (`--recurring-audit`). It validates every source ID and reconciliation count and writes nothing back to Monarch. | Transaction audit: `# / Date / Account / Merchant / Amount / Current Category / Bucket / Recommended Category / Confidence / Evidence`; recurring audit: stable name, aliases, cadence, observations, projection, evidence, status |
...
- **`scripts/prepare-review.sh`** -- Deterministic read-only preparation command. `--include-accounts` emits the complete raw `monarch accounts list --hidden --limit 500` response. Repeat `--account-name-candidate NAME` to resolve exactly one exact-name match and use its ID for the transaction query; zero matches, multiple matches, or combining candidates with `--account` fail clearly. `--include-recurring` emits raw `monarch transactions recurring` output and forwards explicit `--start`/`--end` dates.
- **`scripts/build-audit-output.sh`** -- Deterministic renderer with mutually exclusive input modes. Legacy `--audit` retains the 10-column report-only transaction audit, per-bucket totals, live/saved category validation, and count reconciliation. `--recurring-audit` validates recurring-expense items, exclusions, globally unique transaction IDs, and exact reconciliation counts, then emits recurring items, summary/cross-check/exclusion groups, and count reconciliation. `--validate-only` supports either mode. Run `scripts/build-audit-output.sh --help` for both complete JSON schemas. See the **Renderer Selection** principle for when to use this renderer instead of `build-output.sh`.
