...
- **listings** -- Read and search Mercari listings (read-only):
  ...
  - `get <id>` -- full detail for any listing/item (by id or URL, with `--properties`, `--table`). It reports `buyer_protection_fee_cents` from Mercari's explicit `priceSummary.headline` and `landed_total_cents` from numeric `priceSummary.totalPrice`. The landed total can include buyer-paid shipping and other checkout costs. Both fields use integer cents.
  - `get-many <ids...>` -- full detail for up to 25 items through one browser session. It preserves input order and coverage. Each result uses `status: ok` with `item`, or `status: error` with `error_kind` and `error`. Oversized batches fail before opening the browser. Use `--status-only` for uncached availability fields (`id`, `url`, `status`, and `lastSoldAt` when present) without requiring checkout pricing. Full-detail pricing remains strict, and a pricing parse failure returns a keyed `unreadable` item error while preserving valid siblings. A human verification challenge stops the command.
