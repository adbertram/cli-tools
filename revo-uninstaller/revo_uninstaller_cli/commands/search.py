"""Search commands for RevoUninstaller CLI."""
COMMAND_CREDENTIALS = {
...
from .display import extract_fields, model_to_dict
from cli_tools_shared.output import print_json, print_table, handle_error, print_info
from cli_tools_shared.filters import validate_filters, apply_filters, FilterValidationError

app = typer.Typer(help="Search RevoUninstaller", no_args_is_help=True)
...
def search_query(
    ...
    """
    Search for items on RevoUninstaller.

    Example:
        revo-uninstaller search query "my search term" --limit 20 --table
        revo-uninstaller search query "keyword" --filter "price:lt:100"
        revo-uninstaller search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        revo-uninstaller search item abc123 --table
        revo-uninstaller search item "https://example.com/item/abc123"
        revo-uninstaller search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    """
    List items from RevoUninstaller.

    Example:
        revo-uninstaller search list --limit 100 --table
        revo-uninstaller search list --filter "status:active"
        revo-uninstaller search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        revo-uninstaller search wildcard "*test*"
        revo-uninstaller search wildcard "prefix*"
        revo-uninstaller search wildcard "*pending*" --fields "status,notes"
        revo-uninstaller search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
