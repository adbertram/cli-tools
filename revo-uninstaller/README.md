...
A command-line interface for [RevoUninstaller](https://www.revouninstaller.com/partners/) using browser automation. CLI for Revo Uninstaller partner page
...
After installation, the `revo-uninstaller` command will be available in your terminal.

## Quick Start

```bash
# Login to RevoUninstaller
revo-uninstaller auth login

# Check login status
revo-uninstaller auth status

# Search for items
revo-uninstaller search query "search terms"

# Get item details
revo-uninstaller search item ITEM_ID
```

## Commands

### Authentication (`revo-uninstaller auth`)
...
revo-uninstaller auth login --force

# Check authentication status
revo-uninstaller auth status

# Test authentication against live browser
revo-uninstaller auth test

# Clear stored session
revo-uninstaller auth logout
```
...
revo-uninstaller auth login --profile work

# Use named profile for status check
revo-uninstaller auth status --profile work

# Set default profile via environment variable
export REVO_UNINSTALLER_DEFAULT_PROFILE=work
revo-uninstaller auth status  # Uses 'work' profile

# Logout specific profile
revo-uninstaller auth logout --profile work
...
### Profiles (`revo-uninstaller auth profiles`)

```bash
# List all profiles
revo-uninstaller auth profiles list

# Show default profile
revo-uninstaller auth profiles get default

# Switch default profile
revo-uninstaller auth profiles set-default PROFILE_NAME

# Create a new profile
revo-uninstaller auth profiles create PROFILE_NAME
```

### Cache (`revo-uninstaller cache`)

```bash
# Show cache status
revo-uninstaller cache status

# Clear cached responses
revo-uninstaller cache clear
```

### Search (`revo-uninstaller search`)
...
# Limit results
revo-uninstaller search query "search terms" --limit 10

# Get item details
revo-uninstaller search item ITEM_ID
revo-uninstaller search item https://example.com/item/123

# List all items
revo-uninstaller search list
```
...
REVO_UNINSTALLER_USERNAME=your_username
REVO_UNINSTALLER_PASSWORD=your_password

# Base URL
REVO_UNINSTALLER_BASE_URL=https://www.revouninstaller.com/partners/

# Browser settings (true = invisible, false = visible browser)
REVO_UNINSTALLER_HEADLESS=true

# Authentication Configuration
REVO_UNINSTALLER_AUTH_COOKIE_NAMES=session.*,auth,token,sid  # Regex patterns for auth cookies
REVO_UNINSTALLER_AUTH_SELECTOR=                               # CSS selector indicating authenticated state
REVO_UNINSTALLER_AUTH_URL_PATTERN=                            # URL pattern indicating login page
REVO_UNINSTALLER_AUTH_TIMEOUT=60                              # Seconds to wait for login
REVO_UNINSTALLER_AUTH_POLL_INTERVAL=2                         # Seconds between auth checks
REVO_UNINSTALLER_DEFAULT_PROFILE=default                      # Default profile name
```
...
- **Headless mode**: Set `REVO_UNINSTALLER_HEADLESS=false` to see the browser (useful for debugging)
...
export REVO_UNINSTALLER_HEADLESS=false
revo-uninstaller search query "test"
```
