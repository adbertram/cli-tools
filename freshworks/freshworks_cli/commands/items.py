...
def items_list(
    ...
    Examples:
        ...
        freshworks items list --table
        freshworks items list --limit 10
        freshworks items list --filter "status:active"
        freshworks items list --properties "id,name"
        freshworks items list --properties "id,user.name,status"
    """
    try:
        client = get_client()
...
def items_get(
    ...
    Examples:
        ...
        freshworks items get ITEM_ID --table
        freshworks items get ITEM_ID --properties "id,name"
        freshworks items get ITEM_ID --properties "user.name,user.email"
    """
    try:
        client = get_client()
...
def items_search(
    ...
    Examples:
        freshworks items search "*invoice*"
        freshworks items search "INV-2024*"
        freshworks items search "*pending*" --fields "status,notes"
        freshworks items search "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
