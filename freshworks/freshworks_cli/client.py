"""Freshworks API client with automatic token management and exponential retry."""
...
class FreshworksClient:
    """Client for interacting with Freshworks API with automatic token management and retry."""
    ...
    def __init__(
        ...
        """
        Initialize Freshworks client from configuration.
        ...
        if not self.config.has_credentials():
            ...
            raise ClientError(
                ...
                "Run 'freshworks auth login' to authenticate."
            )
    ...
    def _refresh_token(self):
        ...
        if not refresh_token:
            raise ClientError(
                "No refresh token available. Run 'freshworks auth login' to re-authenticate."
            )
        ...
        # self._update_headers()
        raise ClientError("Token refresh not implemented. Run 'freshworks auth login'.")
    ...
    def _make_request(
        ...
        """
        Make an HTTP request to the Freshworks API with exponential retry.
...
_client: Optional[FreshworksClient] = None


def get_client() -> FreshworksClient:
    """Get or create the global Freshworks client instance."""
    global _client
    if _client is None:
        _client = FreshworksClient()
    return _client
