...
A command-line interface for the [Freshworks API](https://developers.freshworks.com/docs/app-sdk/v3.0/support_ticket/rest-apis/). CLI for Freshworks public API workflows
...
After installation, the `freshworks` command will be available in your terminal.

## Quick Start

```bash
# Authenticate with Freshworks
freshworks auth login

# List items
freshworks items list

# Get a specific item
freshworks items get ITEM_ID
```
...
# Login with API key
freshworks auth login
freshworks auth login --api-key YOUR_API_KEY

# Check authentication status
freshworks auth status
freshworks auth status

# Clear stored credentials
freshworks auth logout

# Run the scaffold auth test
freshworks auth test
```
...
```bash
# List all profiles
freshworks auth profiles list

# Show default profile
freshworks auth profiles get default

# Switch default profile
freshworks auth profiles set-default PROFILE_NAME

# Create a new profile
freshworks auth profiles create PROFILE_NAME
```
...
```bash
# Show cache status
freshworks cache status

# Clear cached responses
freshworks cache clear
```
...
# Limit results
freshworks items list --limit 10
...
```bash
freshworks items list --limit 2
```

### Table Output Example

```bash
freshworks items list --limit 5
```
...
FRESHWORKS_CLIENT_ID=your_client_id
FRESHWORKS_CLIENT_SECRET=your_client_secret

# OAuth tokens (managed automatically after login)
FRESHWORKS_ACCESS_TOKEN=<access_token>
FRESHWORKS_REFRESH_TOKEN=<refresh_token>
FRESHWORKS_TOKEN_EXPIRES_AT=<timestamp>

# Optional: API base URL
FRESHWORKS_BASE_URL=https://www.freshworks.com/
```
...
```bash
freshworks items list | jq '.items[].id'
```

### Export Items to JSON File

```bash
freshworks items list --limit 200 > items.json
```
