...
After installation, the `atlassian` command will be available in your terminal.
...
# Search for items
atlassian search query "search terms"

# Get item details
atlassian search item ITEM_ID
```

## Commands

### Authentication (`atlassian auth`)
...
atlassian auth login --profile work
...
# Limit results
atlassian search query "search terms" --limit 10

# Get item details
atlassian search item ITEM_ID
atlassian search item https://example.com/item/123

# List all items
atlassian search list
```

### Cache (`atlassian cache`)

```bash
# Show cache status for the active profile
atlassian cache status

# Clear cached responses for the active profile
atlassian cache clear
```
...
ATLASSIAN_USERNAME=your_username
ATLASSIAN_PASSWORD=your_password

# Base URL
ATLASSIAN_BASE_URL=https://www.flexoffers.com/affiliate-programs/atlassian-affiliate-program/

# Browser settings (true = invisible, false = visible browser)
ATLASSIAN_HEADLESS=true

# Authentication Configuration
ATLASSIAN_AUTH_COOKIE_NAMES=session.*,auth,token,sid  # Regex patterns for auth cookies
ATLASSIAN_AUTH_SELECTOR=                               # CSS selector indicating authenticated state
...
- **Headless mode**: Set `ATLASSIAN_HEADLESS=false` to see the browser (useful for debugging)
...
export ATLASSIAN_HEADLESS=false
atlassian search query "test"
```
