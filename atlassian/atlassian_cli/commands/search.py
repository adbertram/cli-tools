...
def search_query(
    ...
    Example:
        atlassian search query "my search term" --limit 20 --table
        atlassian search query "keyword" --filter "price:lt:100"
        atlassian search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        atlassian search item abc123 --table
        atlassian search item "https://example.com/item/abc123"
        atlassian search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    Example:
        atlassian search list --limit 100 --table
        atlassian search list --filter "status:active"
        atlassian search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        atlassian search wildcard "*test*"
        atlassian search wildcard "prefix*"
        atlassian search wildcard "*pending*" --fields "status,notes"
        atlassian search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
