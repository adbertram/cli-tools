"""Search commands for Yubico CLI."""
COMMAND_CREDENTIALS = {
...
app = typer.Typer(help="Search Yubico", no_args_is_help=True)
...
def search_query(
    ...
    """
    Search for items on Yubico.

    Example:
        yubico search query "my search term" --limit 20 --table
        yubico search query "keyword" --filter "price:lt:100"
        yubico search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        yubico search item abc123 --table
        yubico search item "https://example.com/item/abc123"
        yubico search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    """
    List items from Yubico.

    Example:
        yubico search list --limit 100 --table
        yubico search list --filter "status:active"
        yubico search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        yubico search wildcard "*test*"
        yubico search wildcard "prefix*"
        yubico search wildcard "*pending*" --fields "status,notes"
        yubico search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
