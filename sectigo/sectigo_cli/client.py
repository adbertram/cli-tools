"""Sectigo client using shared browser session tooling."""
...
class SectigoClient:
    ...
    PRODUCT_NAME = "Sectigo"
    RECORD_ID = "rec6swec5Zp1IcsO7"
    ...
    def _browser_http_client(self) -> BrowserAuthenticatedHttpClient:
        ...
        if self._http_client is None:
            self._http_client = BrowserAuthenticatedHttpClient(
                ...
                allowed_domains=["sectigostore.com", "sectigostore.com"],
                timeout=10,
            )
...
_client: Optional[SectigoClient] = None


def get_client() -> SectigoClient:
    """Get or create the global Sectigo client instance."""
    global _client
    if _client is None:
        _client = SectigoClient()
    return _client
