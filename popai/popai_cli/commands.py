...
app = typer.Typer(help="Search Popai", no_args_is_help=True)
...
def search_query(
    ...
    """
    Search for items on Popai.

    Example:
        popai search query "my search term" --limit 20 --table
        popai search query "keyword" --filter "price:lt:100"
        popai search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        popai search item abc123 --table
        popai search item "https://example.com/item/abc123"
        popai search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    """
    List items from Popai.

    Example:
        popai search list --limit 100 --table
        popai search list --filter "status:active"
        popai search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        popai search wildcard "*test*"
        popai search wildcard "prefix*"
        popai search wildcard "*pending*" --fields "status,notes"
        popai search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
