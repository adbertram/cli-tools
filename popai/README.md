...
A command-line interface for [Popai](https://sheets.popai.pro) using browser automation. Browser automation CLI for PopAi AI Sheets affiliate workflows
...
After installation, the `popai` command will be available in your terminal.
...
# Search for items
popai search query "search terms"

# Get item details
popai search item ITEM_ID
```

## Commands

### Authentication (`popai auth`)
...
popai auth login --profile work

# Use named profile for status check
popai auth status --profile work

# Set default profile via environment variable
export POPAI_DEFAULT_PROFILE=work
popai auth status  # Uses 'work' profile

# Logout specific profile
popai auth logout --profile work
...
### Profiles (`popai auth profiles`)

```bash
# List all profiles
popai auth profiles list

# Show default profile
popai auth profiles get default

# Switch default profile
popai auth profiles set-default PROFILE_NAME

# Create a new profile
popai auth profiles create PROFILE_NAME
```
...
# Limit results
popai search query "search terms" --limit 10

# Get item details
popai search item ITEM_ID
popai search item https://example.com/item/123
...
POPAI_USERNAME=your_username
POPAI_PASSWORD=your_password

# Base URL
POPAI_BASE_URL=https://sheets.popai.pro

# Browser settings (true = invisible, false = visible browser)
POPAI_HEADLESS=true

# Authentication Configuration
POPAI_AUTH_COOKIE_NAMES=session.*,auth,token,sid  # Regex patterns for auth cookies
POPAI_AUTH_SELECTOR=                               # CSS selector indicating authenticated state
POPAI_AUTH_URL_PATTERN=                            # URL pattern indicating login page
POPAI_AUTH_TIMEOUT=60                              # Seconds to wait for login
POPAI_AUTH_POLL_INTERVAL=2                         # Seconds between auth checks
POPAI_DEFAULT_PROFILE=default                      # Default profile name
```
...
- **Headless mode**: Set `POPAI_HEADLESS=false` to see the browser (useful for debugging)
...
export POPAI_HEADLESS=false
popai search query "test"
```
