"""Search commands for Plusmetrica CLI."""
COMMAND_CREDENTIALS = {
...
app = typer.Typer(help="Search Plusmetrica", no_args_is_help=True)
...
def search_query(
    ...
    """
    Search for items on Plusmetrica.

    Example:
        plusmetrica search query "my search term" --limit 20 --table
        plusmetrica search query "keyword" --filter "price:lt:100"
        plusmetrica search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        plusmetrica search item abc123 --table
        plusmetrica search item "https://example.com/item/abc123"
        plusmetrica search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    """
    List items from Plusmetrica.

    Example:
        plusmetrica search list --limit 100 --table
        plusmetrica search list --filter "status:active"
        plusmetrica search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        plusmetrica search wildcard "*test*"
        plusmetrica search wildcard "prefix*"
        plusmetrica search wildcard "*pending*" --fields "status,notes"
        plusmetrica search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
