...
A command-line interface for [Plusmetrica](https://www.plusmetrica.com/) using browser automation. Browser CLI for PlusMetrica workflows
...
After installation, the `plusmetrica` command will be available in your terminal.

## Quick Start

```bash
# Login to Plusmetrica
plusmetrica auth login

# Check login status
plusmetrica auth status

# Search for items
plusmetrica search query "search terms"

# Get item details
plusmetrica search item ITEM_ID
```

## Commands

### Authentication (`plusmetrica auth`)
...
plusmetrica auth login --force

# Check authentication status
plusmetrica auth status

# Test authentication against live browser
plusmetrica auth test

# Clear stored session
plusmetrica auth logout
```
...
plusmetrica auth login --profile work

# Use named profile for status check
plusmetrica auth status --profile work

# Set default profile via environment variable
export PLUSMETRICA_DEFAULT_PROFILE=work
plusmetrica auth status  # Uses 'work' profile

# Logout specific profile
plusmetrica auth logout --profile work
...
### Profiles (`plusmetrica auth profiles`)

```bash
# List all profiles
plusmetrica auth profiles list

# Show default profile
plusmetrica auth profiles get default

# Switch default profile
plusmetrica auth profiles set-default PROFILE_NAME

# Create a new profile
plusmetrica auth profiles create PROFILE_NAME
```

### Cache (`plusmetrica cache`)

```bash
# Show cache status
plusmetrica cache status

# Clear cached responses
plusmetrica cache clear
```

### Search (`plusmetrica search`)
...
# Limit results
plusmetrica search query "search terms" --limit 10

# Get item details
plusmetrica search item ITEM_ID
plusmetrica search item https://example.com/item/123

# List all items
plusmetrica search list
```
...
PLUSMETRICA_USERNAME=your_username
PLUSMETRICA_PASSWORD=your_password

# Base URL
PLUSMETRICA_BASE_URL=https://www.plusmetrica.com/

# Browser settings (true = invisible, false = visible browser)
PLUSMETRICA_HEADLESS=true

# Authentication Configuration
PLUSMETRICA_AUTH_COOKIE_NAMES=session.*,auth,token,sid  # Regex patterns for auth cookies
PLUSMETRICA_AUTH_SELECTOR=                               # CSS selector indicating authenticated state
PLUSMETRICA_AUTH_URL_PATTERN=                            # URL pattern indicating login page
PLUSMETRICA_AUTH_TIMEOUT=60                              # Seconds to wait for login
PLUSMETRICA_AUTH_POLL_INTERVAL=2                         # Seconds between auth checks
PLUSMETRICA_DEFAULT_PROFILE=default                      # Default profile name
```
...
- **Headless mode**: Set `PLUSMETRICA_HEADLESS=false` to see the browser (useful for debugging)
...
export PLUSMETRICA_HEADLESS=false
plusmetrica search query "test"
```
