"""Databox API client with automatic token management and exponential retry."""
...
class DataboxClient:
    """Client for interacting with Databox API with automatic token management and retry."""
    ...
    def __init__(
        ...
        """
        Initialize Databox client from configuration.
        ...
        if not self.config.has_credentials():
            ...
            raise ClientError(
                ...
                "Run 'databox auth login' to authenticate."
            )
    ...
    def _refresh_token(self):
        ...
        if not refresh_token:
            raise ClientError(
                "No refresh token available. Run 'databox auth login' to re-authenticate."
            )
        ...
        # self._update_headers()
        raise ClientError("Token refresh not implemented. Run 'databox auth login'.")
    ...
    def _make_request(
        ...
        """
        Make an HTTP request to the Databox API with exponential retry.
...
_client: Optional[DataboxClient] = None


def get_client() -> DataboxClient:
    """Get or create the global Databox client instance."""
    global _client
    if _client is None:
        _client = DataboxClient()
    return _client
