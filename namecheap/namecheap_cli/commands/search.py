"""Search commands for Namecheap CLI."""
COMMAND_CREDENTIALS = {
    "query": [
        "browser_session"
    ],
...
app = typer.Typer(help="Search Namecheap", no_args_is_help=True)
...
def search_query(
    ...
    """
    Search for items on Namecheap.

    Example:
        namecheap search query "my search term" --limit 20 --table
        namecheap search query "keyword" --filter "price:lt:100"
        namecheap search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        namecheap search item abc123 --table
        namecheap search item "https://example.com/item/abc123"
        namecheap search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    """
    List items from Namecheap.

    Example:
        namecheap search list --limit 100 --table
        namecheap search list --filter "status:active"
        namecheap search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        namecheap search wildcard "*test*"
        namecheap search wildcard "prefix*"
        namecheap search wildcard "*pending*" --fields "status,notes"
        namecheap search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
