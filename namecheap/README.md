...
A command-line interface for [Namecheap](https://www.namecheap.com/affiliates/) using browser automation. CLI interface for Namecheap affiliate browser automation
...
After installation, the `namecheap` command will be available in your terminal.
...
# Search for items
namecheap search query "search terms"

# Get item details
namecheap search item ITEM_ID
```
...
namecheap auth login --profile work

# Use named profile for status check
namecheap auth status --profile work

# Set default profile via environment variable
export NAMECHEAP_DEFAULT_PROFILE=work
namecheap auth status  # Uses 'work' profile

# Logout specific profile
namecheap auth logout --profile work
...
### Profiles (`namecheap auth profiles`)

```bash
# List all profiles
namecheap auth profiles list

# Show default profile
namecheap auth profiles get default

# Switch default profile
namecheap auth profiles set-default PROFILE_NAME

# Create a new profile
namecheap auth profiles create PROFILE_NAME
```

### Search (`namecheap search`)
...
# Limit results
namecheap search query "search terms" --limit 10

# Get item details
namecheap search item ITEM_ID
namecheap search item https://example.com/item/123

# List all items
namecheap search list
```

### Cache (`namecheap cache`)

```bash
# Show cache status for the active profile
namecheap cache status

# Clear cached responses for the active profile
namecheap cache clear
```
...
NAMECHEAP_PASSWORD=your_password

# Base URL
NAMECHEAP_BASE_URL=https://www.namecheap.com/affiliates/

# Browser settings (true = invisible, false = visible browser)
NAMECHEAP_HEADLESS=true

# Authentication Configuration
NAMECHEAP_AUTH_COOKIE_NAMES=session.*,auth,token,sid  # Regex patterns for auth cookies
NAMECHEAP_AUTH_SELECTOR=                               # CSS selector indicating authenticated state
NAMECHEAP_AUTH_URL_PATTERN=                            # URL pattern indicating login page
NAMECHEAP_AUTH_TIMEOUT=60                              # Seconds to wait for login
NAMECHEAP_AUTH_POLL_INTERVAL=2                         # Seconds between auth checks
NAMECHEAP_DEFAULT_PROFILE=default                      # Default profile name
```
...
- **Headless mode**: Set `NAMECHEAP_HEADLESS=false` to see the browser (useful for debugging)
...
export NAMECHEAP_HEADLESS=false
namecheap search query "test"
```
