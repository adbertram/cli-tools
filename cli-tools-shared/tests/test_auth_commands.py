...
def test_single_credential_type_help_does_not_advertise_browser_session():
    config = MagicMock()
    config.CREDENTIAL_TYPES = [CredentialType.OAUTH_AUTHORIZATION_CODE]
    config.AUTH_EXTRA_PROMPTS = []
    config.OAUTH_AUTH_URL = "https://example.com/oauth/authorize"
    config.OAUTH_TOKEN_URL = "https://example.com/oauth/token"
    config.get_browser.return_value = None
    ...
    assert "oauth_authorization_code" in login.output
    assert "browser_session" not in login.output
    assert logout.exit_code == 0, logout.output
    assert "browser sessions" not in logout.output
    assert status.exit_code == 0, status.output
    assert "browser check" not in status.output
