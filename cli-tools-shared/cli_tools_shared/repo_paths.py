...
def find_cli_tools_repo_root(start: Path | None = None) -> Path:
    ...
    for candidate in (probe, *probe.parents):
        if (
            (candidate / "cli-tools-shared" / "pyproject.toml").is_file()
            and (candidate / "secret-manager" / "secrets.sh").is_file()
        ):
            return candidate
    raise RuntimeError(
...
def secret_manager_script(start: Path | None = None) -> Path:
    ...
    script = find_cli_tools_repo_root(start) / "secret-manager" / "secrets.sh"
