"""Search commands for Twopages CLI."""
import fnmatch
import typer
...
app = typer.Typer(help="Search Twopages", no_args_is_help=True)
...
def search_query(
    ...
    """
    Search for items on Twopages.

    Example:
        twopages search query "my search term" --limit 20 --table
        twopages search query "keyword" --filter "price:lt:100"
        twopages search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        twopages search item abc123 --table
        twopages search item "https://example.com/item/abc123"
        twopages search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    """
    List items from Twopages.

    Example:
        twopages search list --limit 100 --table
        twopages search list --filter "status:active"
        twopages search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        twopages search wildcard "*test*"
        twopages search wildcard "prefix*"
        twopages search wildcard "*pending*" --fields "status,notes"
        twopages search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
