...
app = typer.Typer(help="Search Dell", no_args_is_help=True)
...
def search_query(
    ...
    """
    Search for items on Dell.

    Example:
        dell search query "my search term" --limit 20 --table
        dell search query "keyword" --filter "price:lt:100"
        dell search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        dell search item abc123 --table
        dell search item "https://example.com/item/abc123"
        dell search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    """
    List items from Dell.

    Example:
        dell search list --limit 100 --table
        dell search list --filter "status:active"
        dell search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        dell search wildcard "*test*"
        dell search wildcard "prefix*"
        dell search wildcard "*pending*" --fields "status,notes"
        dell search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
