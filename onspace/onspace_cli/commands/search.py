"""Search commands for Onspace CLI."""
import fnmatch
import typer
...
app = typer.Typer(help="Search Onspace", no_args_is_help=True)
...
def search_query(
    ...
    """
    Search for items on Onspace.

    Example:
        onspace search query "my search term" --limit 20 --table
        onspace search query "keyword" --filter "price:lt:100"
        onspace search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        onspace search item abc123 --table
        onspace search item "https://example.com/item/abc123"
        onspace search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    """
    List items from Onspace.

    Example:
        onspace search list --limit 100 --table
        onspace search list --filter "status:active"
        onspace search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        onspace search wildcard "*test*"
        onspace search wildcard "prefix*"
        onspace search wildcard "*pending*" --fields "status,notes"
        onspace search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
