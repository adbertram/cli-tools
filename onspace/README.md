...
A command-line interface for [Onspace](https://www.onspace.ai/imp/ai-app-builder-free) using browser automation. Browser automation CLI for OnSpace AI affiliate workflows
...
After installation, the `onspace` command will be available in your terminal.
...
# Search for items
onspace search query "search terms"

# Get item details
onspace search item ITEM_ID
```

## Commands

### Authentication (`onspace auth`)
...
onspace auth login --profile work

# Use named profile for status check
onspace auth status --profile work

# Set default profile via environment variable
export ONSPACE_DEFAULT_PROFILE=work
onspace auth status  # Uses 'work' profile

# Logout specific profile
onspace auth logout --profile work
...
### Profiles (`onspace auth profiles`)

```bash
# List all profiles
onspace auth profiles list

# Show default profile
onspace auth profiles get default

# Switch default profile
onspace auth profiles set-default PROFILE_NAME

# Create a new profile
onspace auth profiles create PROFILE_NAME
```

### Search (`onspace search`)
...
# Limit results
onspace search query "search terms" --limit 10

# Get item details
onspace search item ITEM_ID
onspace search item https://example.com/item/123
...
ONSPACE_USERNAME=your_username
ONSPACE_PASSWORD=your_password

# Base URL
ONSPACE_BASE_URL=https://www.onspace.ai/imp/ai-app-builder-free

# Browser settings (true = invisible, false = visible browser)
ONSPACE_HEADLESS=true

# Authentication Configuration
ONSPACE_AUTH_COOKIE_NAMES=session.*,auth,token,sid  # Regex patterns for auth cookies
ONSPACE_AUTH_SELECTOR=                               # CSS selector indicating authenticated state
ONSPACE_AUTH_URL_PATTERN=                            # URL pattern indicating login page
ONSPACE_AUTH_TIMEOUT=60                              # Seconds to wait for login
ONSPACE_AUTH_POLL_INTERVAL=2                         # Seconds between auth checks
ONSPACE_DEFAULT_PROFILE=default                      # Default profile name
```
...
- **Headless mode**: Set `ONSPACE_HEADLESS=false` to see the browser (useful for debugging)
...
export ONSPACE_HEADLESS=false
onspace search query "test"
```
