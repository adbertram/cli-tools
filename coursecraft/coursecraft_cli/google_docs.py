...
def _parse_learning_objectives(table_rows: List[Dict]) -> str:
    """Parse learning objectives table into string format matching Airtable.
    ...
    for row in table_rows:
        ...
        # Skip header row
        if 'learning objectives' in key.lower() or 'what are objectives' in key.lower():
            continue

        if key.lower() == 'terminal':
            terminal_count += 1
            if lines:
                lines.append("")  # Add blank line between objectives
            # Strip leading number prefix if present (e.g., "1. Objective" -> "Objective")
            objective_text = re.sub(r'^\d+\.\s*', '', value)
            lines.append(f"Terminal {terminal_count}: {objective_text}")
        elif key.lower() == 'enabling':
            # Split enabling objectives by newline and add as bullet points
            for enabling in value.split('\n'):
                enabling = enabling.strip()
                if enabling:
...
def parse_course_outline(doc_id_or_url: str) -> Dict[str, Any]:
    ...
    for element in content:
        ...
        if table_type == 'course_info':
            ...
            objectives = _parse_learning_objectives(table_rows)
...
# Format: (table_index, row_label, column_index)
COURSE_INFO_TABLE_MAPPING = {
    "Name": (0, "Course Title", 1),
    "Author Name": (0, "Author Name", 1),
    "Course ID": (0, "Opportunity ID", 1),
    "Skill Path": (0, "Skill Path", 1),
    "Path Placement": (0, "Path Placement", 1),
    "Job Role": (0, "Job Role", 1),
    "Content Tags": (0, "Content Tags", 1),
    "Target Length (Min)": (0, "Length Estimate in minutes", 1),
    "Content Level": (0, "Content Level", 1),
    "Notes": (0, "Notes", 1),
}

COURSE_PLANNING_TABLE_MAPPING = {
    "Learner Profile": (1, "Learner Profile", 1),
    "(Required) Learner Prerequisites": (1, "Learner Prerequisites", 1),
    "Storyline": (1, "Storyline", 1),
    "Platform/Tool Versions": (1, "Platform/Tool Versions", 1),
    "Short Description": (1, "Short Description", 1),
    "Long Description": (1, "Long Description", 1),
}


def build_table_updates_from_fields(fields: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
    ...
    for field_name, (table_idx, label, col_idx) in COURSE_INFO_TABLE_MAPPING.items():
    ...
    for field_name, (table_idx, label, col_idx) in COURSE_PLANNING_TABLE_MAPPING.items():
        if field_name not in fields:
            continue
...
def find_course_organization_table_index(doc_id_or_url: str) -> int:
    ...
    for element in content:
        ...
        if not rows:
            continue
        first_cells = rows[0].get('tableCells', [])
        if not first_cells:
            continue
        first_text = _extract_cell_text(first_cells[0]).lower()
        if 'course organization' in first_text:
            return table_idx

    raise RuntimeError(
        "Could not locate 'Course Organization' table in document "
        f"{doc_id}. Tables scanned: {table_idx + 1}"
    )
...
def build_module_table_updates(
    ...
    # Module number = row number (row 0 is header)
    row = module_number
    ...
    if duration is not None:
        # Format duration if just a number
        if duration.isdigit():
            duration = f"{duration} min"
        elif not duration.lower().endswith('min'):
            duration = f"{duration} min"
        updates.append({
            "table": table_index,
            "row": row,
