"""Items commands for Easyship CLI.
...
app = typer.Typer(help="Manage easyship items", no_args_is_help=True)
...
def items_list(
    ...
    Examples:
        easyship items list
        easyship items list --table
        easyship items list --limit 10
        easyship items list --filter "status:active"
        easyship items list --properties "id,name"
        easyship items list --properties "id,user.name,status"
    """
    try:
        client = get_client()
...
def items_get(
    ...
    Examples:
        easyship items get ITEM_ID
        easyship items get ITEM_ID --table
        easyship items get ITEM_ID --properties "id,name"
        easyship items get ITEM_ID --properties "user.name,user.email"
    """
    try:
        client = get_client()
...
def items_search(
    ...
    Examples:
        easyship items search "*invoice*"
        easyship items search "INV-2024*"
        easyship items search "*pending*" --fields "status,notes"
        easyship items search "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
