...
app = typer.Typer(help="Search Opera", no_args_is_help=True)
...
def search_query(
    ...
    """
    Search for items on Opera.

    Example:
        opera search query "my search term" --limit 20 --table
        opera search query "keyword" --filter "price:lt:100"
        opera search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        opera search item abc123 --table
        opera search item "https://example.com/item/abc123"
        opera search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    """
    List items from Opera.

    Example:
        opera search list --limit 100 --table
        opera search list --filter "status:active"
        opera search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        opera search wildcard "*test*"
        opera search wildcard "prefix*"
        opera search wildcard "*pending*" --fields "status,notes"
        opera search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
