...
A command-line interface for [Opera](https://www.opera.com/opera/affiliate) using browser automation. Browser CLI for Opera affiliate workflows
...
After installation, the `opera` command will be available in your terminal.
...
# Search for items
opera search query "search terms"

# Get item details
opera search item ITEM_ID
```

## Commands

### Authentication (`opera auth`)
...
opera auth login --profile work

# Use named profile for status check
opera auth status --profile work

# Set default profile via environment variable
export OPERA_DEFAULT_PROFILE=work
opera auth status  # Uses 'work' profile

# Logout specific profile
opera auth logout --profile work
...
### Profiles (`opera auth profiles`)

```bash
# List all profiles
opera auth profiles list

# Show default profile
opera auth profiles get default

# Switch default profile
opera auth profiles set-default PROFILE_NAME

# Create a new profile
opera auth profiles create PROFILE_NAME
```
...
# Limit results
opera search query "search terms" --limit 10

# Get item details
opera search item ITEM_ID
opera search item https://example.com/item/123
...
# Base URL
OPERA_BASE_URL=https://www.opera.com/opera/affiliate

# Browser settings (true = invisible, false = visible browser)
OPERA_HEADLESS=true

# Authentication Configuration
OPERA_AUTH_COOKIE_NAMES=session.*,auth,token,sid  # Regex patterns for auth cookies
OPERA_AUTH_SELECTOR=                               # CSS selector indicating authenticated state
OPERA_AUTH_URL_PATTERN=                            # URL pattern indicating login page
OPERA_AUTH_TIMEOUT=60                              # Seconds to wait for login
OPERA_AUTH_POLL_INTERVAL=2                         # Seconds between auth checks
OPERA_DEFAULT_PROFILE=default                      # Default profile name
```
...
- **Headless mode**: Set `OPERA_HEADLESS=false` to see the browser (useful for debugging)
...
export OPERA_HEADLESS=false
opera search query "test"
```
