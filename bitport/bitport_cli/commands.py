"""Search commands for Bitport CLI."""
COMMAND_CREDENTIALS = {
...
app = typer.Typer(help="Search Bitport", no_args_is_help=True)
...
def search_query(
    ...
    """
    Search for items on Bitport.

    Example:
        bitport search query "my search term" --limit 20 --table
        bitport search query "keyword" --filter "price:lt:100"
        bitport search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        bitport search item abc123 --table
        bitport search item "https://example.com/item/abc123"
        bitport search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    """
    List items from Bitport.

    Example:
        bitport search list --limit 100 --table
        bitport search list --filter "status:active"
        bitport search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        bitport search wildcard "*test*"
        bitport search wildcard "prefix*"
        bitport search wildcard "*pending*" --fields "status,notes"
        bitport search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
