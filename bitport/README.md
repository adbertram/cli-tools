...
A command-line interface for [Bitport](https://affiliate.bitport.io/) using browser automation. Browser CLI for Bitport affiliate workflows
...
After installation, the `bitport` command will be available in your terminal.
...
# Search for items
bitport search query "search terms"

# Get item details
bitport search item ITEM_ID
```

## Commands

### Authentication (`bitport auth`)
...
bitport auth login --profile work

# Use named profile for status check
bitport auth status --profile work

# Set default profile via environment variable
export BITPORT_DEFAULT_PROFILE=work
bitport auth status  # Uses 'work' profile

# Logout specific profile
bitport auth logout --profile work
...
### Profiles (`bitport auth profiles`)

```bash
# List all profiles
bitport auth profiles list

# Show default profile
bitport auth profiles get default

# Switch default profile
bitport auth profiles set-default PROFILE_NAME

# Create a new profile
bitport auth profiles create PROFILE_NAME
```

### Search (`bitport search`)
...
# Limit results
bitport search query "search terms" --limit 10

# Get item details
bitport search item ITEM_ID
bitport search item https://example.com/item/123
...
BITPORT_USERNAME=your_username
BITPORT_PASSWORD=your_password

# Base URL
BITPORT_BASE_URL=https://affiliate.bitport.io/

# Browser settings (true = invisible, false = visible browser)
BITPORT_HEADLESS=true

# Authentication Configuration
BITPORT_AUTH_COOKIE_NAMES=session.*,auth,token,sid  # Regex patterns for auth cookies
BITPORT_AUTH_SELECTOR=                               # CSS selector indicating authenticated state
BITPORT_AUTH_URL_PATTERN=                            # URL pattern indicating login page
BITPORT_AUTH_TIMEOUT=60                              # Seconds to wait for login
BITPORT_AUTH_POLL_INTERVAL=2                         # Seconds between auth checks
BITPORT_DEFAULT_PROFILE=default                      # Default profile name
```
...
- **Headless mode**: Set `BITPORT_HEADLESS=false` to see the browser (useful for debugging)
...
export BITPORT_HEADLESS=false
bitport search query "test"
```
