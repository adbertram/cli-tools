"""Search commands for MartinicAudio CLI."""
COMMAND_CREDENTIALS = {
...
app = typer.Typer(help="Search MartinicAudio", no_args_is_help=True)
...
def search_query(
    ...
    """
    Search for items on MartinicAudio.

    Example:
        martinic-audio search query "my search term" --limit 20 --table
        martinic-audio search query "keyword" --filter "price:lt:100"
        martinic-audio search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        martinic-audio search item abc123 --table
        martinic-audio search item "https://example.com/item/abc123"
        martinic-audio search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    """
    List items from MartinicAudio.

    Example:
        martinic-audio search list --limit 100 --table
        martinic-audio search list --filter "status:active"
        martinic-audio search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        martinic-audio search wildcard "*test*"
        martinic-audio search wildcard "prefix*"
        martinic-audio search wildcard "*pending*" --fields "status,notes"
        martinic-audio search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
