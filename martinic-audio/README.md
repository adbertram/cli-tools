...
A command-line interface for [MartinicAudio](https://www.martinic.com/en/products) using browser automation. Browser CLI for Martinic Audio product workflows
...
After installation, the `martinic-audio` command will be available in your terminal.

## Quick Start

```bash
# Login to MartinicAudio
martinic-audio auth login

# Check login status
martinic-audio auth status

# Search for items
martinic-audio search query "search terms"

# Get item details
martinic-audio search item ITEM_ID
```

## Commands

### Authentication (`martinic-audio auth`)
...
martinic-audio auth login --force

# Check authentication status
martinic-audio auth status

# Test authentication against live browser
martinic-audio auth test

# Clear stored session
martinic-audio auth logout
```
...
martinic-audio auth login --profile work

# Use named profile for status check
martinic-audio auth status --profile work

# Set default profile via environment variable
export MARTINIC_AUDIO_DEFAULT_PROFILE=work
martinic-audio auth status  # Uses 'work' profile

# Logout specific profile
martinic-audio auth logout --profile work
...
### Profiles (`martinic-audio auth profiles`)

```bash
# List all profiles
martinic-audio auth profiles list

# Show default profile
martinic-audio auth profiles get default

# Switch default profile
martinic-audio auth profiles set-default PROFILE_NAME

# Create a new profile
martinic-audio auth profiles create PROFILE_NAME
```

### Search (`martinic-audio search`)
...
# Limit results
martinic-audio search query "search terms" --limit 10

# Get item details
martinic-audio search item ITEM_ID
martinic-audio search item https://example.com/item/123

# List all items
martinic-audio search list
```

### Cache (`martinic-audio cache`)

```bash
# Show cache status for the active profile
martinic-audio cache status

# Clear cached responses for the active profile
martinic-audio cache clear
```
...
MARTINIC_AUDIO_USERNAME=your_username
MARTINIC_AUDIO_PASSWORD=your_password

# Base URL
MARTINIC_AUDIO_BASE_URL=https://www.martinic.com/en/products

# Browser settings (true = invisible, false = visible browser)
MARTINIC_AUDIO_HEADLESS=true

# Authentication Configuration
MARTINIC_AUDIO_AUTH_COOKIE_NAMES=session.*,auth,token,sid  # Regex patterns for auth cookies
MARTINIC_AUDIO_AUTH_SELECTOR=                               # CSS selector indicating authenticated state
MARTINIC_AUDIO_AUTH_URL_PATTERN=                            # URL pattern indicating login page
MARTINIC_AUDIO_AUTH_TIMEOUT=60                              # Seconds to wait for login
MARTINIC_AUDIO_AUTH_POLL_INTERVAL=2                         # Seconds between auth checks
MARTINIC_AUDIO_DEFAULT_PROFILE=default                      # Default profile name
```
...
- **Headless mode**: Set `MARTINIC_AUDIO_HEADLESS=false` to see the browser (useful for debugging)
...
export MARTINIC_AUDIO_HEADLESS=false
martinic-audio search query "test"
```
