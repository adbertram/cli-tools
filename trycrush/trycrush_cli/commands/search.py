"""Search commands for Trycrush CLI."""
COMMAND_CREDENTIALS = {
...
app = typer.Typer(help="Search Trycrush", no_args_is_help=True)
...
def search_query(
    ...
    """
    Search for items on Trycrush.

    Example:
        trycrush search query "my search term" --limit 20 --table
        trycrush search query "keyword" --filter "price:lt:100"
        trycrush search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        trycrush search item abc123 --table
        trycrush search item "https://example.com/item/abc123"
        trycrush search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    """
    List items from Trycrush.

    Example:
        trycrush search list --limit 100 --table
        trycrush search list --filter "status:active"
        trycrush search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        trycrush search wildcard "*test*"
        trycrush search wildcard "prefix*"
        trycrush search wildcard "*pending*" --fields "status,notes"
        trycrush search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
