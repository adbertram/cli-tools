...
A command-line interface for [Trycrush](https://trycrush.ai) using browser automation. Browser CLI for TryCrush workflows
...
After installation, the `trycrush` command will be available in your terminal.
...
# Check login status
trycrush auth status

# Search for items
trycrush search query "search terms"

# Get item details
trycrush search item ITEM_ID
```

## Commands

### Authentication (`trycrush auth`)
...
trycrush auth login --profile work

# Use named profile for status check
trycrush auth status --profile work

# Set default profile via environment variable
export TRYCRUSH_DEFAULT_PROFILE=work
trycrush auth status  # Uses 'work' profile

# Logout specific profile
trycrush auth logout --profile work
...
### Profiles (`trycrush auth profiles`)

```bash
# List all profiles
trycrush auth profiles list

# Show default profile
trycrush auth profiles get default

# Switch default profile
trycrush auth profiles set-default PROFILE_NAME

# Create a new profile
trycrush auth profiles create PROFILE_NAME
```

### Cache (`trycrush cache`)

```bash
# Show cache status
trycrush cache status

# Clear cached responses
trycrush cache clear
```

### Search (`trycrush search`)
...
# Limit results
trycrush search query "search terms" --limit 10

# Get item details
trycrush search item ITEM_ID
trycrush search item https://example.com/item/123

# List all items
trycrush search list
```
...
TRYCRUSH_USERNAME=your_username
TRYCRUSH_PASSWORD=your_password

# Base URL
TRYCRUSH_BASE_URL=https://trycrush.ai

# Browser settings (true = invisible, false = visible browser)
TRYCRUSH_HEADLESS=true

# Authentication Configuration
TRYCRUSH_AUTH_COOKIE_NAMES=session.*,auth,token,sid  # Regex patterns for auth cookies
TRYCRUSH_AUTH_SELECTOR=                               # CSS selector indicating authenticated state
TRYCRUSH_AUTH_URL_PATTERN=                            # URL pattern indicating login page
TRYCRUSH_AUTH_TIMEOUT=60                              # Seconds to wait for login
TRYCRUSH_AUTH_POLL_INTERVAL=2                         # Seconds between auth checks
TRYCRUSH_DEFAULT_PROFILE=default                      # Default profile name
```
...
- **Headless mode**: Set `TRYCRUSH_HEADLESS=false` to see the browser (useful for debugging)
...
export TRYCRUSH_HEADLESS=false
trycrush search query "test"
```
