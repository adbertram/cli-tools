"""Search commands for MetaBox CLI."""
COMMAND_CREDENTIALS = {
...
app = typer.Typer(help="Search MetaBox", no_args_is_help=True)
...
def search_query(
    ...
    """
    Search for items on MetaBox.

    Example:
        meta-box search query "my search term" --limit 20 --table
        meta-box search query "keyword" --filter "price:lt:100"
        meta-box search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        meta-box search item abc123 --table
        meta-box search item "https://example.com/item/abc123"
        meta-box search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    """
    List items from MetaBox.

    Example:
        meta-box search list --limit 100 --table
        meta-box search list --filter "status:active"
        meta-box search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        meta-box search wildcard "*test*"
        meta-box search wildcard "prefix*"
        meta-box search wildcard "*pending*" --fields "status,notes"
        meta-box search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
