"""Configuration management for Pluralsight CLI."""
...
class Config(BaseConfig):
    ...
    CREDENTIAL_TYPES = []
    DEFAULT_BASE_URL = "https://api-us1.cludo.com/api/v3/10000847/10001278"
    # Uncomment when auth login needs required non-secret config first.
    # AUTH_CONFIG_PROMPTS = [("BASE_URL", "Pluralsight base URL", False)]
    ...
    def test_connection(self) -> dict:
        """Validate API reachability with a live call (public endpoint, no credentials)."""
        from .client import PluralsightClient

        try:
            PluralsightClient(config=self).list_items(page=1, per_page=1)
