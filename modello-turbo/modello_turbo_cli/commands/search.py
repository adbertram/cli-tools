"""Search commands for ModelloTurbo CLI."""
COMMAND_CREDENTIALS = {
...
app = typer.Typer(help="Search ModelloTurbo", no_args_is_help=True)
...
def search_query(
    ...
    """
    Search for items on ModelloTurbo.

    Example:
        modello-turbo search query "my search term" --limit 20 --table
        modello-turbo search query "keyword" --filter "price:lt:100"
        modello-turbo search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        modello-turbo search item abc123 --table
        modello-turbo search item "https://example.com/item/abc123"
        modello-turbo search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    """
    List items from ModelloTurbo.

    Example:
        modello-turbo search list --limit 100 --table
        modello-turbo search list --filter "status:active"
        modello-turbo search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        modello-turbo search wildcard "*test*"
        modello-turbo search wildcard "prefix*"
        modello-turbo search wildcard "*pending*" --fields "status,notes"
        modello-turbo search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
