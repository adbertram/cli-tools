...
A command-line interface for [ModelloTurbo](https://modelloturbo.com/) using browser automation. CLI for Modello Turbo site
...
After installation, the `modello-turbo` command will be available in your terminal.

## Quick Start

```bash
# Login to ModelloTurbo
modello-turbo auth login

# Check login status
modello-turbo auth status

# Search for items
modello-turbo search query "search terms"

# Get item details
modello-turbo search item ITEM_ID
```

## Commands

### Authentication (`modello-turbo auth`)
...
modello-turbo auth login --force

# Check authentication status
modello-turbo auth status

# Test authentication against live browser
modello-turbo auth test

# Clear stored session
modello-turbo auth logout
```
...
modello-turbo auth login --profile work

# Use named profile for status check
modello-turbo auth status --profile work

# Set default profile via environment variable
export MODELLO_TURBO_DEFAULT_PROFILE=work
modello-turbo auth status  # Uses 'work' profile

# Logout specific profile
modello-turbo auth logout --profile work
...
### Profiles (`modello-turbo auth profiles`)

```bash
# List all profiles
modello-turbo auth profiles list

# Show default profile
modello-turbo auth profiles get default

# Switch default profile
modello-turbo auth profiles set-default PROFILE_NAME

# Create a new profile
modello-turbo auth profiles create PROFILE_NAME
```

### Cache (`modello-turbo cache`)

```bash
# Show cache status
modello-turbo cache status

# Clear cached responses
modello-turbo cache clear
```

### Search (`modello-turbo search`)
...
# Limit results
modello-turbo search query "search terms" --limit 10

# Get item details
modello-turbo search item ITEM_ID
modello-turbo search item https://example.com/item/123

# List all items
modello-turbo search list
```
...
MODELLO_TURBO_USERNAME=your_username
MODELLO_TURBO_PASSWORD=your_password

# Base URL
MODELLO_TURBO_BASE_URL=https://modelloturbo.com/

# Browser settings (true = invisible, false = visible browser)
MODELLO_TURBO_HEADLESS=true

# Authentication Configuration
MODELLO_TURBO_AUTH_COOKIE_NAMES=session.*,auth,token,sid  # Regex patterns for auth cookies
MODELLO_TURBO_AUTH_SELECTOR=                               # CSS selector indicating authenticated state
MODELLO_TURBO_AUTH_URL_PATTERN=                            # URL pattern indicating login page
MODELLO_TURBO_AUTH_TIMEOUT=60                              # Seconds to wait for login
MODELLO_TURBO_AUTH_POLL_INTERVAL=2                         # Seconds between auth checks
MODELLO_TURBO_DEFAULT_PROFILE=default                      # Default profile name
```
...
- **Headless mode**: Set `MODELLO_TURBO_HEADLESS=false` to see the browser (useful for debugging)
...
export MODELLO_TURBO_HEADLESS=false
modello-turbo search query "test"
```
