# MicrosoftAdvertising CLI

A command-line interface for [MicrosoftAdvertising](https://signup.cj.com/member/signup/publisher/?cid=3065612) using browser automation. CLI for Microsoft Advertising affiliate signup surface

## Installation

```bash
cd microsoft-advertising
pip install -e .
...
After installation, the `microsoft-advertising` command will be available in your terminal.

## Quick Start

```bash
# Login to MicrosoftAdvertising
microsoft-advertising auth login

# Check login status
microsoft-advertising auth status

# Search for items
microsoft-advertising search query "search terms"

# Get item details
microsoft-advertising search item ITEM_ID
```

## Commands

### Authentication (`microsoft-advertising auth`)
...
microsoft-advertising auth login --force

# Check authentication status
microsoft-advertising auth status

# Test authentication against live browser
microsoft-advertising auth test

# Clear stored session
microsoft-advertising auth logout
```
...
microsoft-advertising auth login --profile work

# Use named profile for status check
microsoft-advertising auth status --profile work

# Set default profile via environment variable
export MICROSOFT_ADVERTISING_DEFAULT_PROFILE=work
microsoft-advertising auth status  # Uses 'work' profile

# Logout specific profile
microsoft-advertising auth logout --profile work
...
### Profiles (`microsoft-advertising auth profiles`)

```bash
# List all profiles
microsoft-advertising auth profiles list

# Show default profile
microsoft-advertising auth profiles get default

# Switch default profile
microsoft-advertising auth profiles set-default PROFILE_NAME

# Create a new profile
microsoft-advertising auth profiles create PROFILE_NAME
```

### Cache (`microsoft-advertising cache`)

```bash
# Show cache status
microsoft-advertising cache status

# Clear cached responses
microsoft-advertising cache clear
```

### Search (`microsoft-advertising search`)
...
# Limit results
microsoft-advertising search query "search terms" --limit 10

# Get item details
microsoft-advertising search item ITEM_ID
microsoft-advertising search item https://example.com/item/123

# List all items
microsoft-advertising search list
```
...
MICROSOFT_ADVERTISING_USERNAME=your_username
MICROSOFT_ADVERTISING_PASSWORD=your_password

# Base URL
MICROSOFT_ADVERTISING_BASE_URL=https://signup.cj.com/member/signup/publisher/?cid=3065612

# Browser settings (true = invisible, false = visible browser)
MICROSOFT_ADVERTISING_HEADLESS=true

# Authentication Configuration
MICROSOFT_ADVERTISING_AUTH_COOKIE_NAMES=session.*,auth,token,sid  # Regex patterns for auth cookies
MICROSOFT_ADVERTISING_AUTH_SELECTOR=                               # CSS selector indicating authenticated state
MICROSOFT_ADVERTISING_AUTH_URL_PATTERN=                            # URL pattern indicating login page
MICROSOFT_ADVERTISING_AUTH_TIMEOUT=60                              # Seconds to wait for login
MICROSOFT_ADVERTISING_AUTH_POLL_INTERVAL=2                         # Seconds between auth checks
MICROSOFT_ADVERTISING_DEFAULT_PROFILE=default                      # Default profile name
```
...
- **Headless mode**: Set `MICROSOFT_ADVERTISING_HEADLESS=false` to see the browser (useful for debugging)
...
export MICROSOFT_ADVERTISING_HEADLESS=false
microsoft-advertising search query "test"
```
