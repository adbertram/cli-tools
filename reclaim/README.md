...
A command-line interface for [Reclaim](https://reclaim.ai/) using browser automation. CLI for Reclaim.ai affiliate program surface
...
# Search for items
reclaim search query "search terms"

# Get item details
reclaim search item ITEM_ID
```

## Commands

### Authentication (`reclaim auth`)
...
reclaim auth login --profile work

# Use named profile for status check
reclaim auth status --profile work

# Set default profile via environment variable
export RECLAIM_DEFAULT_PROFILE=work
reclaim auth status  # Uses 'work' profile

# Logout specific profile
reclaim auth logout --profile work
...
### Profiles (`reclaim auth profiles`)

```bash
# List all profiles
reclaim auth profiles list

# Show default profile
reclaim auth profiles get default

# Switch default profile
reclaim auth profiles set-default PROFILE_NAME

# Create a new profile
reclaim auth profiles create PROFILE_NAME
```
...
```bash
# Show cache status
reclaim cache status

# Clear cached responses
reclaim cache clear
```

### Search (`reclaim search`)
...
# Limit results
reclaim search query "search terms" --limit 10

# Get item details
reclaim search item ITEM_ID
reclaim search item https://example.com/item/123
...
RECLAIM_USERNAME=your_username
RECLAIM_PASSWORD=your_password

# Base URL
RECLAIM_BASE_URL=https://reclaim.ai/

# Browser settings (true = invisible, false = visible browser)
RECLAIM_HEADLESS=true

# Authentication Configuration
RECLAIM_AUTH_COOKIE_NAMES=session.*,auth,token,sid  # Regex patterns for auth cookies
RECLAIM_AUTH_SELECTOR=                               # CSS selector indicating authenticated state
RECLAIM_AUTH_URL_PATTERN=                            # URL pattern indicating login page
RECLAIM_AUTH_TIMEOUT=60                              # Seconds to wait for login
RECLAIM_AUTH_POLL_INTERVAL=2                         # Seconds between auth checks
RECLAIM_DEFAULT_PROFILE=default                      # Default profile name
```
...
- **Headless mode**: Set `RECLAIM_HEADLESS=false` to see the browser (useful for debugging)
...
export RECLAIM_HEADLESS=false
reclaim search query "test"
```
