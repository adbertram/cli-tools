"""Search commands for Reclaim CLI."""
COMMAND_CREDENTIALS = {
...
app = typer.Typer(help="Search Reclaim", no_args_is_help=True)
...
def search_query(
    ...
    """
    Search for items on Reclaim.

    Example:
        reclaim search query "my search term" --limit 20 --table
        reclaim search query "keyword" --filter "price:lt:100"
        reclaim search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        reclaim search item abc123 --table
        reclaim search item "https://example.com/item/abc123"
        reclaim search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    """
    List items from Reclaim.

    Example:
        reclaim search list --limit 100 --table
        reclaim search list --filter "status:active"
        reclaim search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        reclaim search wildcard "*test*"
        reclaim search wildcard "prefix*"
        reclaim search wildcard "*pending*" --fields "status,notes"
        reclaim search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
