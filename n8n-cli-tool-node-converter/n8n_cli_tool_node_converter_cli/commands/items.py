"""Items commands for N8nCliToolNodeConverter CLI.
...
app = typer.Typer(help="Manage n8n-cli-tool-node-converter items", no_args_is_help=True)
...
def items_list(
    ...
    Examples:
        n8n-cli-tool-node-converter items list
        n8n-cli-tool-node-converter items list --table
        n8n-cli-tool-node-converter items list --limit 10
        n8n-cli-tool-node-converter items list --filter "status:active"
        n8n-cli-tool-node-converter items list --properties "id,name"
        n8n-cli-tool-node-converter items list --properties "id,user.name,status"
    """
    try:
        client = get_client()
...
def items_get(
    ...
    Examples:
        n8n-cli-tool-node-converter items get ITEM_ID
        n8n-cli-tool-node-converter items get ITEM_ID --table
        n8n-cli-tool-node-converter items get ITEM_ID --properties "id,name"
        n8n-cli-tool-node-converter items get ITEM_ID --properties "user.name,user.email"
    """
    try:
        client = get_client()
...
def items_search(
    ...
    Examples:
        n8n-cli-tool-node-converter items search "*invoice*"
        n8n-cli-tool-node-converter items search "INV-2024*"
        n8n-cli-tool-node-converter items search "*pending*" --fields "status,notes"
        n8n-cli-tool-node-converter items search "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
