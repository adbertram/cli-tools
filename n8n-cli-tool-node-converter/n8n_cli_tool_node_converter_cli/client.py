"""File-based client for n8n CLI Tool Node Converter.
...
class N8nConverterClient:
    ...
    def __init__(self):
        ...
        if not self.config.has_credentials():
            ...
            raise ClientError(
                ...
                "Run 'n8n-cli-tool-node-converter auth login' to configure."
            )
    ...
    def generate(self, tool_name: str, output_dir: Optional[str] = None, force: bool = False) -> str:
        ...
        try:
            return generate_node_package(metadata, target_dir, force=force)
    ...
    def list_generated(self) -> List[GeneratedPackage]:
        ...
        for child in sorted(output_path.iterdir()):
            if not child.is_dir():
                continue
            if not child.name.startswith("n8n-nodes-"):
                continue
            ...
            cli_name = child.name.replace("n8n-nodes-", "")
