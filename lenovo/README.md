...
A command-line interface for [Lenovo](https://www.lenovo.com/us/en/affiliate-program/resources/index.html) using browser automation. Browser automation for Lenovo affiliate resource workflows
...
After installation, the `lenovo` command will be available in your terminal.
...
# Search for items
lenovo search query "search terms"

# Get item details
lenovo search item ITEM_ID
```

## Commands

### Authentication (`lenovo auth`)
...
lenovo auth login --profile work

# Use named profile for status check
lenovo auth status --profile work

# Set default profile via environment variable
export LENOVO_DEFAULT_PROFILE=work
lenovo auth status  # Uses 'work' profile

# Logout specific profile
lenovo auth logout --profile work
...
### Profiles (`lenovo auth profiles`)

```bash
# List all profiles
lenovo auth profiles list

# Show default profile
lenovo auth profiles get default

# Switch default profile
lenovo auth profiles set-default PROFILE_NAME

# Create a new profile
lenovo auth profiles create PROFILE_NAME
```

### Search (`lenovo search`)
...
# Limit results
lenovo search query "search terms" --limit 10

# Get item details
lenovo search item ITEM_ID
lenovo search item https://example.com/item/123
...
LENOVO_USERNAME=your_username
LENOVO_PASSWORD=your_password

# Base URL
LENOVO_BASE_URL=https://www.lenovo.com/us/en/affiliate-program/resources/index.html

# Browser settings (true = invisible, false = visible browser)
LENOVO_HEADLESS=true

# Authentication Configuration
LENOVO_AUTH_COOKIE_NAMES=session.*,auth,token,sid  # Regex patterns for auth cookies
LENOVO_AUTH_SELECTOR=                               # CSS selector indicating authenticated state
LENOVO_AUTH_URL_PATTERN=                            # URL pattern indicating login page
LENOVO_AUTH_TIMEOUT=60                              # Seconds to wait for login
LENOVO_AUTH_POLL_INTERVAL=2                         # Seconds between auth checks
LENOVO_DEFAULT_PROFILE=default                      # Default profile name
```
...
- **Headless mode**: Set `LENOVO_HEADLESS=false` to see the browser (useful for debugging)
...
export LENOVO_HEADLESS=false
lenovo search query "test"
```
