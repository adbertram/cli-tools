"""Search commands for Lenovo CLI."""
import fnmatch
import typer
...
app = typer.Typer(help="Search Lenovo", no_args_is_help=True)
...
def search_query(
    ...
    """
    Search for items on Lenovo.

    Example:
        lenovo search query "my search term" --limit 20 --table
        lenovo search query "keyword" --filter "price:lt:100"
        lenovo search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        lenovo search item abc123 --table
        lenovo search item "https://example.com/item/abc123"
        lenovo search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    """
    List items from Lenovo.

    Example:
        lenovo search list --limit 100 --table
        lenovo search list --filter "status:active"
        lenovo search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        lenovo search wildcard "*test*"
        lenovo search wildcard "prefix*"
        lenovo search wildcard "*pending*" --fields "status,notes"
        lenovo search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
