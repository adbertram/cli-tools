"""LinkedinLearning client using shared browser session tooling."""
...
class LinkedinLearningClient:
    """Minimal client for verified program metadata."""

    PRODUCT_NAME = "LinkedIn Learning"
    RECORD_ID = "rec3RcbaoLjeaAmg4"
    ...
    def _browser_http_client(self) -> BrowserAuthenticatedHttpClient:
        ...
        if self._http_client is None:
            self._http_client = BrowserAuthenticatedHttpClient(
                ...
                allowed_domains=["learning.linkedin.com", "linkedin.com"],
                timeout=10,
            )
...
_client: Optional[LinkedinLearningClient] = None


def get_client() -> LinkedinLearningClient:
    """Get or create the global LinkedinLearning client instance."""
    global _client
    if _client is None:
        _client = LinkedinLearningClient()
    return _client
