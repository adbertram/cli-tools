"""Search commands for HideMe CLI."""
COMMAND_CREDENTIALS = {
...
app = typer.Typer(help="Search HideMe", no_args_is_help=True)
...
def search_query(
    ...
    """
    Search for items on HideMe.

    Example:
        hide-me search query "my search term" --limit 20 --table
        hide-me search query "keyword" --filter "price:lt:100"
        hide-me search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        hide-me search item abc123 --table
        hide-me search item "https://example.com/item/abc123"
        hide-me search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    """
    List items from HideMe.

    Example:
        hide-me search list --limit 100 --table
        hide-me search list --filter "status:active"
        hide-me search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        hide-me search wildcard "*test*"
        hide-me search wildcard "prefix*"
        hide-me search wildcard "*pending*" --fields "status,notes"
        hide-me search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
