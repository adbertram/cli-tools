"""TunnelBear client using shared browser session tooling."""
...
class TunnelBearClient:
    ...
    PRODUCT_NAME = "TunnelBear"
    RECORD_ID = "rec8f6lzrmyjq8o7I"
    ...
    def _browser_http_client(self) -> BrowserAuthenticatedHttpClient:
        ...
        if self._http_client is None:
            self._http_client = BrowserAuthenticatedHttpClient(
                ...
                allowed_domains=["www.tunnelbear.com", "tunnelbear.com"],
                timeout=10,
            )
...
_client: Optional[TunnelBearClient] = None


def get_client() -> TunnelBearClient:
    """Get or create the global TunnelBear client instance."""
    global _client
    if _client is None:
        _client = TunnelBearClient()
    return _client
