...
<header>
  ...
  <label class="meta" title="Sets with a computed net profit under $10, AND sets with no profit figure at all (no comps, missing landed cost, gated incomplete), are hidden by default. Uncheck to see everything."><input type="checkbox" id="minprofit" checked> hide sets &lt; $10 / unpriced</label>
  <label class="meta" title="Listings priced under $20 are not worth the time to chase -- hidden by default, bulk and sets both. Uses the Price column's number (the current bid on a bid-only auction)."><input type="checkbox" id="minprice" checked> hide price &lt; $20</label>
</header>
...
const MAXBID_TIP="The most this lot is worth to Adam -- $4.00/lb scaled by the quality multiplier for bulk, or net resale less the minimum margin for a set. The score collapses as the price approaches it.";
...
// toggle rather than another status chip: its own columns, its own render, and
// no action buttons -- prospects are read-only here by design.
...
const COLS=[{k:"score",l:"Score",s:1},{k:"title",l:"Title",w:"width:99%"},{k:"cat",l:"Cat"},
  ...
  {k:"ends",l:"Ends"},{k:"tlb",l:"Target $/lb"},{k:"act",l:""}];
...
// $/lb Adam types, times the lot's weight) -> the hammer bid that reaches it.
...
function calcBid(r,target){
  ...
  if(!isFinite(t)||t<=0||!r.weight)return null;
  const total=t*r.weight;
...
function tlbOut(r,val){
  ...
  if(bid==null)return"";
  return bid<0?'<span class="neg" title="Fixed fees and shipping alone exceed this $/lb target.">over budget</span>':money(bid);
}
...
function rows(){const q=document.getElementById("q").value.toLowerCase();
  let d=DATA.filter(r=>{
    ...
    if(minProfitOnly&&filter!=="needs_review"&&r.cat==="set"&&(r.profit==null||r.profit<10))return false;
...
function profitCell(r){
  ...
  const star=r.cat==="set"&&r.auc?'<span class="dim" title="'+esc(MAXBID_PROFIT_TIP)+'">*</span>':"";
  const zc=r.cat==="set"&&r.zeroCompNote?'<span class="dim" title="'+esc(r.zeroCompNote)+'">&dagger;</span>':"";
...
function render(){const d=rows();
  ...
  document.getElementById("tbody").innerHTML=d.map(r=>{
    ...
    const tlbCell=(r.cat==="bulk"&&r.weight&&r.ltype!=="fixed")
      ?'<td class="tlb"><input type="number" step="0.01" min="0" inputmode="decimal" placeholder="$/lb" '
  ...
  document.getElementById("catfilters").innerHTML=["all","bulk","set"].map(v=>
...
function renderProspects(){
  ...
  document.getElementById("tbody").innerHTML=PDATA.map(r=>'<tr>'+PCOLS.map(c=>{
    const v=r[c.k];
    if(c.k==="name")return '<td class="tc"><a href="'+esc(r.citation_url)+'" target="_blank" rel="noopener noreferrer">'+esc(v)+'</a></td>';
    ...
    return '<td'+(c.k==="created_at"?' class="dim"':"")+'>'+esc(v)+'</td>';}).join("")+'</tr>').join("");
}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        # /status and /favorite are the ONLY write routes. Prospects are
        # read-only structurally: there is no prospect write handler, so POST
        # /prospects.json is a 404.
