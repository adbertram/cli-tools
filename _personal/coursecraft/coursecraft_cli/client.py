...
class CourseCraftClient:
    ...
    def list_fields(self, table: str) -> List[Dict]:
        ...
        for field in fields:
            if not isinstance(field, dict):
                raise ClientError(
                    f"Field metadata for table {table} must contain objects; got {field!r}."
                )
            row = {key: field.get(key) for key in ("id", "name", "type")}
            missing = [key for key, value in row.items() if not isinstance(value, str)]
            if missing:
                raise ClientError(
                    f"Field metadata for table {table} is missing string "
                    f"{', '.join(missing)}: {field!r}"
                )
            rows.append(row)
        return rows
    ...
    def rename_field(self, table: str, current_name: str, new_name: str) -> Dict:
        ...
        current = [field for field in fields if field["name"] == current_name]
        if len(current) != 1:
            raise ClientError(
                f"Expected exactly one field named '{current_name}' in {table}; "
                f"found {len(current)}."
            )
        if any(field["name"] == new_name for field in fields):
            raise ClientError(f"Field '{new_name}' already exists in {table}.")

        field_id = current[0]["id"]
    ...
    def _warn_disabled_course(self, course: Dict[str, Any]) -> None:
        ...
        if not isinstance(course, dict):
            raise ClientError(
                "Disabled-course reminder needs a course record object; got "
                f"{type(course).__name__}."
            )
        fields = course.get("fields", {})
    ...
    def _course_from_record(self, table: str, record: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        if not isinstance(record, dict):
            raise ClientError(
                f"Cannot resolve the owning course of a {table} record from a "
                f"{type(record).__name__}; a record object is required."
            )
