...
# course-pipeline skill's v3-field-assignment.md and SKILL.md). Anything not
# listed here (slide.narration, clip.recording) has no folder-root field on
# its own table yet and is intentionally NOT registered by this command in
# this phase.
...
# Provenance sidecar for a Linux demo's committed container image. It is an
# ordinary text file in the demo's Folder Root, so it registers through the
# same file-kind path as everything else here -- the image itself is never
# hashed by this command, and no Docker access happens inside `sync`.
ENVIRONMENT_IMAGE_FILENAME = "environment-image.json"
...
def _seed_demo_files(fields: Dict[str, Any], existing_vc: Dict[str, Any]) -> Dict[str, Any]:
    ...
    env_prep = _register_file(
        "demo.environment_prep_script", folder / env_prep_filename, existing_vc
    )
    if env_prep is not None:
        updates["demo.environment_prep_script"] = env_prep

    env_image = _register_file(
        "demo.environment_image", folder / ENVIRONMENT_IMAGE_FILENAME, existing_vc
    )
    if env_image is not None:
        updates["demo.environment_image"] = env_image
    ...
    dictation = _register_file(
        "demo.dictation_audio", folder / dictation_filename, existing_vc
    )
    if dictation is not None:
        updates["demo.dictation_audio"] = dictation

    return updates


def _seed_demo_walkthrough_file(
    fields: Dict[str, Any], existing_vc: Dict[str, Any], module_order: Any, clip_order: Any
) -> Dict[str, Any]:
    """The promoted final demo video, at ``<Folder Root>/m{M}c{C}-demo.mp4``."""
    folder_root = fields.get("Folder Root")
    if not isinstance(folder_root, str) or not folder_root.strip():
        return {}
    if module_order is None or clip_order is None:
        return {}
    slug = (
        "demo.automated_walkthrough"
        if fields.get("Execution Method") == AUTOMATED_WALKTHROUGH_EXECUTION_METHOD
        else "demo.manual_walkthrough"
    )
    folder = resolve_course_folder(folder_root)
    filename = f"m{module_order}c{clip_order}-demo.mp4"
    registration = _register_file(slug, folder / filename, existing_vc)
    return {slug: registration} if registration is not None else {}
...
def _seed_module_deck_file(fields: Dict[str, Any], existing_vc: Dict[str, Any]) -> Dict[str, Any]:
    """Register the canonical deck for the module's current lifecycle state."""
    path = _module_deck_path(fields)
    if path is None:
        return {}
    registration = _register_file("module.powerpoint_deck", path, existing_vc)
    return {"module.powerpoint_deck": registration} if registration is not None else {}
...
def versions_sync(
    ...
    changed files for the file-kind slugs this command resolves a path for
    (demo.environment_prep_script, demo.dictation_audio,
    demo.automated_walkthrough/demo.manual_walkthrough,
    ...
    try:
        ...
        for table, record, module_order, clip_order in _walk_course(client, course_record):
            ...
            try:
                ...
                if table == "Demos":
                    updates.update(_seed_demo_files(fields, existing_vc))
                    updates.update(
                        _seed_demo_walkthrough_file(fields, existing_vc, module_order, clip_order)
                    )
                if table == "Modules":
                    deck_updates = _seed_module_deck_file(fields, existing_vc)
                    updates.update(deck_updates)
                    if deck_updates:
