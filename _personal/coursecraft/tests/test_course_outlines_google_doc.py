...
from coursecraft_cli.commands import course_outlines
from coursecraft_cli.commands.course_outlines import _parse_markdown_to_outline_update
    ...
    def get_record(self, table, record_id):
        return {
            ...
            "fields": {
                ...
                "Pluralsight Course Outline Link": (
...
def _current_template_doc(monkeypatch):
    document = _document_with_shifted_outline_tables()
    monkeypatch.setattr(course_outlines, "get_client", lambda: _FakeClient())
    monkeypatch.setattr(google_docs, "get_document_structure", lambda _doc_id: document)
    monkeypatch.setattr(
        course_outlines, "get_document_structure", lambda _doc_id: document
    )
    return document
...
def test_validate_only_agrees_with_the_write_path(_current_template_doc, monkeypatch):
    ...
    monkeypatch.setattr(course_outlines, "_apply_table_updates", _refuse_write)
...
def test_verifier_rejects_a_cell_that_holds_more_than_was_written(monkeypatch):
    ...
    with pytest.raises(RuntimeError) as excinfo:
        course_outlines._verify_table_updates(
            "doc-under-test",
...
def test_zero_applied_cells_is_a_hard_failure(monkeypatch):
    """`updates: 0` means the document was not changed, so it cannot be success."""
    class _Result:
        returncode = 0
        stdout = '{"documentId": "doc-under-test", "updates": 0}'
        stderr = ""

    monkeypatch.setattr(course_outlines.subprocess, "run", lambda *a, **k: _Result())
    monkeypatch.setattr(course_outlines, "_verify_table_updates", lambda *a, **k: None)

    with pytest.raises(RuntimeError) as excinfo:
        course_outlines._apply_table_updates(
            "doc-under-test",
            [{"table": 0, "row": 0, "col": 1, "content": "PARA ONE"}],
        )

    assert "applied 0 of 1 requested table cell update(s)" in str(excinfo.value)
