"""Categories commands for ATA Blog CLI, backed by the static site's terms.json."""
import typer
...
app = typer.Typer(help="Manage static site categories")
...
@command
def categories_get(
    term_id: int = typer.Argument(..., help="Category ID"),
    table: bool = typer.Option(False, "--table", "-t", help="Display as table"),
):
...
@app.command("create")
@command
def categories_create(name: str = typer.Argument(..., help="Category name")):
    """Create a category in the static site's terms.json."""
    term = create_term(TAXONOMY, name)
    print_success(f"Created category {term['name']} (id {term['id']})")
    print_json(term)
