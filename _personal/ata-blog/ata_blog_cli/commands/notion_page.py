...
def articles_list(
    ...
    Date properties (Publish Date, Stage Date) use Notion's date operators:
    equals, before, after, on_or_before, on_or_after, this_week, past_week,
    past_month, past_year, next_week, next_month, next_year, is_empty,
    is_not_empty. Aliases: gt=after, gte=on_or_after, lt=before,
    lte=on_or_before.

        ata-blog notion-page list --filter "Publish Date:gte:2026-07-20"
        ata-blog notion-page list --filter "Publish Date:past_week"
        ata-blog notion-page list --filter "Publish Date:is_not_empty:true"
    """
    client = get_client()
...
def articles_publish(
    ...
    status: str = typer.Option(
        ...
        "--status",
        "-s",
        help="Publish status (draft/preview/publish). preview deploys to Cloudflare Pages without mutating Notion; publish promotes to production",
    ),
    ...
    """Schedule a Notion article, deploy a non-publishing Cloudflare preview, or promote it to production."""
    client = get_client()
    ...
    if result.get("status") == "preview":
        print_success(
            f"Cloudflare preview deployed (Deployment ID: {result['deployment_id']})"
        )
...
def articles_search(
    ...
    if not articles:
        print_info(f"NO_MATCH: No articles found matching '{query}'")
        if not table:
            print_json([])
        return
