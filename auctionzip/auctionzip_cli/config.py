"""Configuration management for AuctionZip CLI (browser automation).
...
class Config(BaseConfig):
    """Configuration for AuctionZip — extends BaseConfig for shared auth/profile support."""

    DIST_NAME = "auctionzip-cli"

    CREDENTIAL_TYPES = [CredentialType.BROWSER_SESSION]
    DEFAULT_BASE_URL = "https://www.auctionzip.com"
    ...
    def browser_user_agent(self) -> str:
        ...
        auctionzip.com sits behind Cloudflare; the default ``HeadlessChrome/<v>``
    ...
    def get_browser(self):
        ...
        from .browser import AuctionzipBrowser
        return AuctionzipBrowser(self)

    def test_connection(self) -> dict:
        """Live browser-session verification used by `auctionzip auth test`.

        Navigates the persistent session to auctionzip.com and returns the
        shared browser session test result (proves Cloudflare clearance is live).
        """
