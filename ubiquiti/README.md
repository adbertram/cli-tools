...
A command-line interface for [Ubiquiti](https://creators.ui.com/) using browser automation. CLI for Ubiquiti Creator Program
...
After installation, the `ubiquiti` command will be available in your terminal.
...
# Check login status
ubiquiti auth status

# Search for items
ubiquiti search query "search terms"

# Get item details
ubiquiti search item ITEM_ID
```

## Commands

### Authentication (`ubiquiti auth`)
...
ubiquiti auth login --profile work

# Use named profile for status check
ubiquiti auth status --profile work

# Set default profile via environment variable
export UBIQUITI_DEFAULT_PROFILE=work
ubiquiti auth status  # Uses 'work' profile

# Logout specific profile
ubiquiti auth logout --profile work
...
### Profiles (`ubiquiti auth profiles`)

```bash
# List all profiles
ubiquiti auth profiles list

# Show default profile
ubiquiti auth profiles get default

# Switch default profile
ubiquiti auth profiles set-default PROFILE_NAME

# Create a new profile
ubiquiti auth profiles create PROFILE_NAME
```

### Cache (`ubiquiti cache`)

```bash
# Show cache status
ubiquiti cache status

# Clear cached responses
ubiquiti cache clear
```

### Search (`ubiquiti search`)
...
# Limit results
ubiquiti search query "search terms" --limit 10

# Get item details
ubiquiti search item ITEM_ID
ubiquiti search item https://example.com/item/123

# List all items
ubiquiti search list
```
...
UBIQUITI_USERNAME=your_username
UBIQUITI_PASSWORD=your_password

# Base URL
UBIQUITI_BASE_URL=https://creators.ui.com/

# Browser settings (true = invisible, false = visible browser)
UBIQUITI_HEADLESS=true

# Authentication Configuration
UBIQUITI_AUTH_COOKIE_NAMES=session.*,auth,token,sid  # Regex patterns for auth cookies
UBIQUITI_AUTH_SELECTOR=                               # CSS selector indicating authenticated state
UBIQUITI_AUTH_URL_PATTERN=                            # URL pattern indicating login page
UBIQUITI_AUTH_TIMEOUT=60                              # Seconds to wait for login
UBIQUITI_AUTH_POLL_INTERVAL=2                         # Seconds between auth checks
UBIQUITI_DEFAULT_PROFILE=default                      # Default profile name
```
...
- **Headless mode**: Set `UBIQUITI_HEADLESS=false` to see the browser (useful for debugging)
...
export UBIQUITI_HEADLESS=false
ubiquiti search query "test"
```
