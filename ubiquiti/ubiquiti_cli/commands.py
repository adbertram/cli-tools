"""Search commands for Ubiquiti CLI."""
COMMAND_CREDENTIALS = {
...
app = typer.Typer(help="Search Ubiquiti", no_args_is_help=True)
...
def search_query(
    ...
    """
    Search for items on Ubiquiti.

    Example:
        ubiquiti search query "my search term" --limit 20 --table
        ubiquiti search query "keyword" --filter "price:lt:100"
        ubiquiti search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        ubiquiti search item abc123 --table
        ubiquiti search item "https://example.com/item/abc123"
        ubiquiti search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    """
    List items from Ubiquiti.

    Example:
        ubiquiti search list --limit 100 --table
        ubiquiti search list --filter "status:active"
        ubiquiti search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        ubiquiti search wildcard "*test*"
        ubiquiti search wildcard "prefix*"
        ubiquiti search wildcard "*pending*" --fields "status,notes"
        ubiquiti search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
