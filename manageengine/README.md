...
A command-line interface for [Manageengine](https://www.manageengine.com/affiliate/signup.html) using browser automation. CLI for ManageEngine affiliate signup page
...
After installation, the `manageengine` command will be available in your terminal.

## Quick Start

```bash
# Login to Manageengine
manageengine auth login

# Check login status
manageengine auth status

# Search for items
manageengine search query "search terms"

# Get item details
manageengine search item ITEM_ID
```

## Commands

### Authentication (`manageengine auth`)
...
manageengine auth login --force

# Check authentication status
manageengine auth status

# Test authentication against live browser
manageengine auth test

# Clear stored session
manageengine auth logout
```
...
manageengine auth login --profile work

# Use named profile for status check
manageengine auth status --profile work

# Set default profile via environment variable
export MANAGEENGINE_DEFAULT_PROFILE=work
manageengine auth status  # Uses 'work' profile

# Logout specific profile
manageengine auth logout --profile work
...
### Profiles (`manageengine auth profiles`)

```bash
# List all profiles
manageengine auth profiles list

# Show default profile
manageengine auth profiles get default

# Switch default profile
manageengine auth profiles set-default PROFILE_NAME

# Create a new profile
manageengine auth profiles create PROFILE_NAME
```

### Cache (`manageengine cache`)

```bash
# Show cache status
manageengine cache status

# Clear cached responses
manageengine cache clear
```

### Search (`manageengine search`)
...
# Limit results
manageengine search query "search terms" --limit 10

# Get item details
manageengine search item ITEM_ID
manageengine search item https://example.com/item/123

# List all items
manageengine search list
```
...
MANAGEENGINE_USERNAME=your_username
MANAGEENGINE_PASSWORD=your_password

# Base URL
MANAGEENGINE_BASE_URL=https://www.manageengine.com/affiliate/signup.html

# Browser settings (true = invisible, false = visible browser)
MANAGEENGINE_HEADLESS=true

# Authentication Configuration
MANAGEENGINE_AUTH_COOKIE_NAMES=session.*,auth,token,sid  # Regex patterns for auth cookies
MANAGEENGINE_AUTH_SELECTOR=                               # CSS selector indicating authenticated state
MANAGEENGINE_AUTH_URL_PATTERN=                            # URL pattern indicating login page
MANAGEENGINE_AUTH_TIMEOUT=60                              # Seconds to wait for login
MANAGEENGINE_AUTH_POLL_INTERVAL=2                         # Seconds between auth checks
MANAGEENGINE_DEFAULT_PROFILE=default                      # Default profile name
```
...
- **Headless mode**: Set `MANAGEENGINE_HEADLESS=false` to see the browser (useful for debugging)
...
export MANAGEENGINE_HEADLESS=false
manageengine search query "test"
```
