...
def search_query(
    ...
    Example:
        manageengine search query "my search term" --limit 20 --table
        manageengine search query "keyword" --filter "price:lt:100"
        manageengine search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        manageengine search item abc123 --table
        manageengine search item "https://example.com/item/abc123"
        manageengine search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    Example:
        manageengine search list --limit 100 --table
        manageengine search list --filter "status:active"
        manageengine search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        manageengine search wildcard "*test*"
        manageengine search wildcard "prefix*"
        manageengine search wildcard "*pending*" --fields "status,notes"
        manageengine search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
