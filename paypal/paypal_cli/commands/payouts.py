...
def payouts_list(
    ...
    limit: int = typer.Option(None, "--limit", "-l", help="Maximum number of payouts to return"),
    filter_str: str = typer.Option(None, "--filter", "-f", help="Filter: field:op:value (e.g., name:eq:MyItem, status:contains:active)"),
    properties: str = typer.Option(None, "--properties", "-p", help="Comma-separated list of properties to include"),
):
    """List batch payouts (not yet implemented via API)."""
    print_info("Payout listing via API is not yet implemented.")
    raise typer.Exit(1)
...
def payouts_create(
    ...
    try:
        ...
        print_info(f"Invalid JSON in --items-json: {e}")
        raise typer.Exit(1)
...
def payouts_get(
    ...
    try:
        ...
        if table:
            ...
            print_table(rows, ["batch_id", "status", "amount", "currency", "fees"],
            ...
            if items:
                ...
                print_table(item_rows, ["item_id", "status", "receiver", "amount"],
                            ["Item ID", "Status", "Receiver", "Amount"])
        else:
            print_json(result)
    except ClientError as e:
        raise typer.Exit(handle_error(e))
...
def payouts_get_item(
    ...
    try:
        ...
        if table:
            ...
            print_table(rows, ["item_id", "status", "receiver", "amount", "currency", "fee"],
