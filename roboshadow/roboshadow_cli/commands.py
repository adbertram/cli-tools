"""Search commands for Roboshadow CLI."""
COMMAND_CREDENTIALS = {
...
app = typer.Typer(help="Search Roboshadow", no_args_is_help=True)
...
def search_query(
    ...
    """
    Search for items on Roboshadow.

    Example:
        roboshadow search query "my search term" --limit 20 --table
        roboshadow search query "keyword" --filter "price:lt:100"
        roboshadow search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        roboshadow search item abc123 --table
        roboshadow search item "https://example.com/item/abc123"
        roboshadow search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    """
    List items from Roboshadow.

    Example:
        roboshadow search list --limit 100 --table
        roboshadow search list --filter "status:active"
        roboshadow search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        roboshadow search wildcard "*test*"
        roboshadow search wildcard "prefix*"
        roboshadow search wildcard "*pending*" --fields "status,notes"
        roboshadow search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
