...
A command-line interface for [Roboshadow](https://www.roboshadow.com/) using browser automation. Browser CLI for RoboShadow account workflows
...
After installation, the `roboshadow` command will be available in your terminal.

## Quick Start

```bash
# Login to Roboshadow
roboshadow auth login

# Check login status
roboshadow auth status

# Search for items
roboshadow search query "search terms"

# Get item details
roboshadow search item ITEM_ID
```

## Commands

### Authentication (`roboshadow auth`)
...
roboshadow auth login --force

# Check authentication status
roboshadow auth status

# Test authentication against live browser
roboshadow auth test

# Clear stored session
roboshadow auth logout
```
...
roboshadow auth login --profile work

# Use named profile for status check
roboshadow auth status --profile work

# Set default profile via environment variable
export ROBOSHADOW_DEFAULT_PROFILE=work
roboshadow auth status  # Uses 'work' profile

# Logout specific profile
roboshadow auth logout --profile work
...
### Profiles (`roboshadow auth profiles`)

```bash
# List all profiles
roboshadow auth profiles list

# Show default profile
roboshadow auth profiles get default

# Switch default profile
roboshadow auth profiles set-default PROFILE_NAME

# Create a new profile
roboshadow auth profiles create PROFILE_NAME
```

### Search (`roboshadow search`)
...
# Limit results
roboshadow search query "search terms" --limit 10

# Get item details
roboshadow search item ITEM_ID
roboshadow search item https://example.com/item/123

# List all items
roboshadow search list
```

### Cache (`roboshadow cache`)

```bash
# Show cache status for the active profile
roboshadow cache status

# Clear cached responses for the active profile
roboshadow cache clear
```
...
ROBOSHADOW_USERNAME=your_username
ROBOSHADOW_PASSWORD=your_password

# Base URL
ROBOSHADOW_BASE_URL=https://www.roboshadow.com/

# Browser settings (true = invisible, false = visible browser)
ROBOSHADOW_HEADLESS=true

# Authentication Configuration
ROBOSHADOW_AUTH_COOKIE_NAMES=session.*,auth,token,sid  # Regex patterns for auth cookies
ROBOSHADOW_AUTH_SELECTOR=                               # CSS selector indicating authenticated state
ROBOSHADOW_AUTH_URL_PATTERN=                            # URL pattern indicating login page
ROBOSHADOW_AUTH_TIMEOUT=60                              # Seconds to wait for login
ROBOSHADOW_AUTH_POLL_INTERVAL=2                         # Seconds between auth checks
ROBOSHADOW_DEFAULT_PROFILE=default                      # Default profile name
```
...
- **Headless mode**: Set `ROBOSHADOW_HEADLESS=false` to see the browser (useful for debugging)
...
export ROBOSHADOW_HEADLESS=false
roboshadow search query "test"
```
