"""Items commands for Digicert CLI.
...
app = typer.Typer(help="Manage digicert items", no_args_is_help=True)
...
def items_list(
    ...
    Examples:
        digicert items list
        digicert items list --table
        digicert items list --limit 10
        digicert items list --filter "status:active"
        digicert items list --properties "id,name"
        digicert items list --properties "id,user.name,status"
    """
    try:
        client = get_client()
...
def items_get(
    ...
    Examples:
        ...
        digicert items get ITEM_ID --table
        digicert items get ITEM_ID --properties "id,name"
        digicert items get ITEM_ID --properties "user.name,user.email"
    """
    try:
        client = get_client()
...
def items_search(
    ...
    Examples:
        digicert items search "*invoice*"
        digicert items search "INV-2024*"
        digicert items search "*pending*" --fields "status,notes"
        digicert items search "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
