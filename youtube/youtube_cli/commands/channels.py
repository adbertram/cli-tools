...
def _list_owned_channels(profile: Optional[str], limit: Optional[int] = None) -> List[dict]:
    ...
    while True:
        ...
        channels.extend(_normalize_channel(item) for item in response.get("items", []))
...
def list_channels(
    ...
    """List YouTube channels available to the authenticated account."""
    try:
        validate_filters(filter or [])
        rows = _list_owned_channels(profile, limit=limit)
...
def get_channel(
    ...
    """Get one owned YouTube channel by channel ID."""
    try:
        rows = _list_owned_channels(profile)
    except HttpError as exc:
        print_error(f"HTTP error: {exc}")
        raise typer.Exit(1)
    except Exception as exc:
