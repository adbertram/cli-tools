...
A command-line interface for the official [Rewarx affiliate program page](https://www.rewarx.com/).
...
### Program (`rewarx program`)

```bash
rewarx program info
rewarx program info --table
```

### Authentication (`rewarx auth`)

```bash
rewarx auth login
rewarx auth login --force
rewarx auth status
rewarx auth test
rewarx auth logout
rewarx auth profiles list
```
...
- It exposes verified program metadata for the official Rewarx URL.
