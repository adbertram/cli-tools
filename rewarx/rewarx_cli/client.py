"""Rewarx client using shared browser session tooling."""
    ...
    def _browser_http_client(self) -> BrowserAuthenticatedHttpClient:
        ...
        if self._http_client is None:
            self._http_client = BrowserAuthenticatedHttpClient(
                ...
                allowed_domains=["www.rewarx.com", "rewarx.com"],
                timeout=10,
            )
...
_client: Optional[RewarxClient] = None


def get_client() -> RewarxClient:
    """Get or create the global Rewarx client instance."""
    global _client
    if _client is None:
        _client = RewarxClient()
    return _client
