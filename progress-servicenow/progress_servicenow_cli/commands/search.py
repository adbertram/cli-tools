"""Search commands for ProgressServicenow CLI."""
import fnmatch
import typer
...
app = typer.Typer(help="Search ProgressServicenow", no_args_is_help=True)
...
def search_query(
    ...
    """
    Search for items on ProgressServicenow.

    Example:
        progress-servicenow search query "my search term" --limit 20 --table
        progress-servicenow search query "keyword" --filter "price:lt:100"
        progress-servicenow search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        progress-servicenow search item abc123 --table
        progress-servicenow search item "https://example.com/item/abc123"
        progress-servicenow search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    """
    List items from ProgressServicenow.

    Example:
        progress-servicenow search list --limit 100 --table
        progress-servicenow search list --filter "status:active"
        progress-servicenow search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        progress-servicenow search wildcard "*test*"
        progress-servicenow search wildcard "prefix*"
        progress-servicenow search wildcard "*pending*" --fields "status,notes"
        progress-servicenow search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
