"""Search commands for Nvidia CLI."""
import fnmatch
import typer
...
app = typer.Typer(help="Search Nvidia", no_args_is_help=True)
...
def search_query(
    ...
    """
    Search for items on Nvidia.

    Example:
        nvidia search query "my search term" --limit 20 --table
        nvidia search query "keyword" --filter "price:lt:100"
        nvidia search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        nvidia search item abc123 --table
        nvidia search item "https://example.com/item/abc123"
        nvidia search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    """
    List items from Nvidia.

    Example:
        nvidia search list --limit 100 --table
        nvidia search list --filter "status:active"
        nvidia search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        nvidia search wildcard "*test*"
        nvidia search wildcard "prefix*"
        nvidia search wildcard "*pending*" --fields "status,notes"
        nvidia search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
