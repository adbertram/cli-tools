...
A command-line interface for [Nvidia](https://www.nvidia.com/en-us/affiliates/) using browser automation. Browser automation for NVIDIA affiliate program workflows
...
After installation, the `nvidia` command will be available in your terminal.
...
# Search for items
nvidia search query "search terms"

# Get item details
nvidia search item ITEM_ID
```

## Commands

### Authentication (`nvidia auth`)
...
nvidia auth login --profile work

# Use named profile for status check
nvidia auth status --profile work

# Set default profile via environment variable
export NVIDIA_DEFAULT_PROFILE=work
nvidia auth status  # Uses 'work' profile

# Logout specific profile
nvidia auth logout --profile work
...
### Profiles (`nvidia auth profiles`)

```bash
# List all profiles
nvidia auth profiles list

# Show default profile
nvidia auth profiles get default

# Switch default profile
nvidia auth profiles set-default PROFILE_NAME

# Create a new profile
nvidia auth profiles create PROFILE_NAME
```

### Search (`nvidia search`)
...
# Limit results
nvidia search query "search terms" --limit 10

# Get item details
nvidia search item ITEM_ID
nvidia search item https://example.com/item/123
...
NVIDIA_USERNAME=your_username
NVIDIA_PASSWORD=your_password

# Base URL
NVIDIA_BASE_URL=https://www.nvidia.com/en-us/affiliates/

# Browser settings (true = invisible, false = visible browser)
NVIDIA_HEADLESS=true

# Authentication Configuration
NVIDIA_AUTH_COOKIE_NAMES=session.*,auth,token,sid  # Regex patterns for auth cookies
NVIDIA_AUTH_SELECTOR=                               # CSS selector indicating authenticated state
NVIDIA_AUTH_URL_PATTERN=                            # URL pattern indicating login page
NVIDIA_AUTH_TIMEOUT=60                              # Seconds to wait for login
NVIDIA_AUTH_POLL_INTERVAL=2                         # Seconds between auth checks
NVIDIA_DEFAULT_PROFILE=default                      # Default profile name
```
...
- **Headless mode**: Set `NVIDIA_HEADLESS=false` to see the browser (useful for debugging)
...
export NVIDIA_HEADLESS=false
nvidia search query "test"
```
