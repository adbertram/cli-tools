"""Search commands for Dropxl CLI."""
COMMAND_CREDENTIALS = {
...
app = typer.Typer(help="Search Dropxl", no_args_is_help=True)
...
def search_query(
    ...
    """
    Search for items on Dropxl.

    Example:
        dropxl search query "my search term" --limit 20 --table
        dropxl search query "keyword" --filter "price:lt:100"
        dropxl search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        dropxl search item abc123 --table
        dropxl search item "https://example.com/item/abc123"
        dropxl search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    """
    List items from Dropxl.

    Example:
        dropxl search list --limit 100 --table
        dropxl search list --filter "status:active"
        dropxl search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        dropxl search wildcard "*test*"
        dropxl search wildcard "prefix*"
        dropxl search wildcard "*pending*" --fields "status,notes"
        dropxl search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
