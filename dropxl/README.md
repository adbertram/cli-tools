...
A command-line interface for [Dropxl](https://www.dropxl.com/homepage.html) using browser automation. Browser CLI for DropXL account workflows
...
After installation, the `dropxl` command will be available in your terminal.
...
# Search for items
dropxl search query "search terms"

# Get item details
dropxl search item ITEM_ID
```

## Commands

### Authentication (`dropxl auth`)
...
dropxl auth login --profile work

# Use named profile for status check
dropxl auth status --profile work

# Set default profile via environment variable
export DROPXL_DEFAULT_PROFILE=work
dropxl auth status  # Uses 'work' profile

# Logout specific profile
dropxl auth logout --profile work
...
### Profiles (`dropxl auth profiles`)

```bash
# List all profiles
dropxl auth profiles list

# Show default profile
dropxl auth profiles get default

# Switch default profile
dropxl auth profiles set-default PROFILE_NAME

# Create a new profile
dropxl auth profiles create PROFILE_NAME
```

### Search (`dropxl search`)
...
# Limit results
dropxl search query "search terms" --limit 10

# Get item details
dropxl search item ITEM_ID
dropxl search item https://example.com/item/123
...
DROPXL_USERNAME=your_username
DROPXL_PASSWORD=your_password

# Base URL
DROPXL_BASE_URL=https://www.dropxl.com/homepage.html

# Browser settings (true = invisible, false = visible browser)
DROPXL_HEADLESS=true

# Authentication Configuration
DROPXL_AUTH_COOKIE_NAMES=session.*,auth,token,sid  # Regex patterns for auth cookies
DROPXL_AUTH_SELECTOR=                               # CSS selector indicating authenticated state
DROPXL_AUTH_URL_PATTERN=                            # URL pattern indicating login page
DROPXL_AUTH_TIMEOUT=60                              # Seconds to wait for login
DROPXL_AUTH_POLL_INTERVAL=2                         # Seconds between auth checks
DROPXL_DEFAULT_PROFILE=default                      # Default profile name
```
...
- **Headless mode**: Set `DROPXL_HEADLESS=false` to see the browser (useful for debugging)
...
export DROPXL_HEADLESS=false
dropxl search query "test"
```
