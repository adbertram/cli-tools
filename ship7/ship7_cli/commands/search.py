...
app = typer.Typer(help="Search Ship7", no_args_is_help=True)
...
def search_query(
    ...
    """
    Search for items on Ship7.

    Example:
        ship7 search query "my search term" --limit 20 --table
        ship7 search query "keyword" --filter "price:lt:100"
        ship7 search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        ship7 search item abc123 --table
        ship7 search item "https://example.com/item/abc123"
        ship7 search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    """
    List items from Ship7.

    Example:
        ship7 search list --limit 100 --table
        ship7 search list --filter "status:active"
        ship7 search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        ship7 search wildcard "*test*"
        ship7 search wildcard "prefix*"
        ship7 search wildcard "*pending*" --fields "status,notes"
        ship7 search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
