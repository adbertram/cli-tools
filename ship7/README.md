...
A command-line interface for [Ship7](https://www.ship7.com) using browser automation. Browser automation for Ship7 site workflows
...
After installation, the `ship7` command will be available in your terminal.
...
# Search for items
ship7 search query "search terms"

# Get item details
ship7 search item ITEM_ID
```

## Commands

### Authentication (`ship7 auth`)
...
ship7 auth login --profile work

# Use named profile for status check
ship7 auth status --profile work

# Set default profile via environment variable
export SHIP7_DEFAULT_PROFILE=work
ship7 auth status  # Uses 'work' profile

# Logout specific profile
ship7 auth logout --profile work
...
### Profiles (`ship7 auth profiles`)

```bash
# List all profiles
ship7 auth profiles list

# Show default profile
ship7 auth profiles get default

# Switch default profile
ship7 auth profiles set-default PROFILE_NAME

# Create a new profile
ship7 auth profiles create PROFILE_NAME
```
...
# Limit results
ship7 search query "search terms" --limit 10

# Get item details
ship7 search item ITEM_ID
ship7 search item https://example.com/item/123
...
SHIP7_USERNAME=your_username
SHIP7_PASSWORD=your_password

# Base URL
SHIP7_BASE_URL=https://www.ship7.com

# Browser settings (true = invisible, false = visible browser)
SHIP7_HEADLESS=true

# Authentication Configuration
SHIP7_AUTH_COOKIE_NAMES=session.*,auth,token,sid  # Regex patterns for auth cookies
SHIP7_AUTH_SELECTOR=                               # CSS selector indicating authenticated state
SHIP7_AUTH_URL_PATTERN=                            # URL pattern indicating login page
SHIP7_AUTH_TIMEOUT=60                              # Seconds to wait for login
SHIP7_AUTH_POLL_INTERVAL=2                         # Seconds between auth checks
SHIP7_DEFAULT_PROFILE=default                      # Default profile name
```
...
- **Headless mode**: Set `SHIP7_HEADLESS=false` to see the browser (useful for debugging)
...
export SHIP7_HEADLESS=false
ship7 search query "test"
```
