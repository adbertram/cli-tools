"""MeasureUp client using shared browser session tooling."""
...
class MeasureUpClient:
    """Minimal client for verified program metadata."""

    PRODUCT_NAME = "MeasureUp"
    RECORD_ID = "rec8cDuEKTCI25Kqg"
    ...
    def _browser_http_client(self) -> BrowserAuthenticatedHttpClient:
        ...
        if self._http_client is None:
            self._http_client = BrowserAuthenticatedHttpClient(
                ...
                allowed_domains=["www.measureup.com", "measureup.com"],
                timeout=10,
            )
...
_client: Optional[MeasureUpClient] = None


def get_client() -> MeasureUpClient:
    """Get or create the global MeasureUp client instance."""
    global _client
    if _client is None:
        _client = MeasureUpClient()
    return _client
