...
class FacebookClient:
    ...
    def _verify_comment_landed(
        ...
        if composer_cleared:
            ...
            # Per Adam's standing instruction, treat this as likely-success
            # rather than a hard failure — callers will record it accordingly.
            return {
    ...
    def close(self):
        ...
        if self._browser_instance is not None:
            self._browser_instance = None
            logger.debug("close: released browser; process teardown closes Playwright")
    ...
    def _iter_relay_prefetched_stream_results(self, body: str):
        """Yield RelayPrefetchedStreamCache result objects from Facebook HTML."""
        for value in iter_json_values_at_marker(body, '["RelayPrefetchedStreamCache","next"'):
            if (
    ...
    def _group_post_from_story_node(self, group_id: str, node: Dict) -> Dict:
        ...
        return {
            ...
            "text": self._extract_group_post_text(node),
            "timestamp": timestamp,
            "url": (
    ...
    def _extract_group_discussion_variables(self, body: str) -> Dict:
        """Extract the current group discussion Relay variables from bootstrap HTML."""
        marker = f'"queryID":"{GROUP_DISCUSSION_DOC_ID}","variables":'
        start = body.find(marker)
        if start < 0:
            raise ClientError("Facebook group discussion Relay preloader variables were not found.")
        try:
            variables, _ = json.JSONDecoder().raw_decode(body, start + len(marker))
        except json.JSONDecodeError as exc:
            raise ClientError("Facebook group discussion Relay preloader variables are malformed.") from exc
        if not isinstance(variables, dict):
            raise ClientError("Facebook group discussion Relay preloader variables are not an object.")
        return variables
    ...
    def _graphql_group_discussion_posts(
        ...
        variables = self._extract_group_discussion_variables(body)
        ...
        relay_request = RelayFormRequest(
            ...
            document_id=GROUP_DISCUSSION_DOC_ID,
            variables=variables,
            base_fields={
                "av": user_id,
                "__user": user_id,
        ...
        return self._extract_group_discussion_posts(group_id, payloads)

    def _extract_group_discussion_posts(self, group_id: str, payloads: tuple[Dict, ...]) -> tuple[List[Dict], bool]:
        ...
        for payload in payloads:
            ...
            if errors:
                raise ClientError(f"Facebook group feed GraphQL returned errors: {errors}")
    ...
    def _extract_comments_from_rendered_page(self, url: str, post_id: str) -> List[Dict]:
        """Render the post permalink in the real browser and scrape comments.

        Facebook lazy-loads replies. We navigate to the permalink (which opens a
        post dialog), click every "View N replies" / "View more replies" button
        until no new ones appear, then parse every role="article" element
        inside the dialog.
        ...
        for art in articles:
            ...
            # at the top of every reply (e.g. "Adam Bertram\nReply text ...").
    ...
    def _extract_group_posts(self, page) -> List[Dict]:
        ...
        js = (
            ...
            '   seen.add(postId);'
            '   posts.push({post_id: postId, author: authorText, text: text.substring(0, 500),'
            '     url: postUrl, reactions: reactions, comment_count: comments});'
            ' }'
            ' return posts;'
            ' }'
        )
    ...
    def list_group_posts(self, group_id: str, limit: int = 20) -> List[GroupPost]:
        """List posts from a Facebook Group."""
        page = self._get_page(f"{GROUPS_BASE}/{group_id}/")
        items: List[Dict] = []
        seen_ids = set()
        previous_count = -1

        for _ in range(8):
            for post in self._extract_group_posts(page):
        ...
        if not items:
            raise ClientError("Facebook group feed DOM did not expose any posts.")
    ...
    def get_group_post(self, post_ref: str) -> GroupPost:
        ...
        if post_ref.startswith("http"):
            ...
            url = f"https://www.facebook.com/groups/{post_ref}"
        ...
        body = self._fetch_authenticated_facebook_full_html(url)
        data = None
        for result in self._iter_relay_prefetched_stream_results(body):
            node = self._find_story_node_by_post_id(result, post_id)
            if node is not None:
                data = self._group_post_from_story_node(group_id, node)
                break

        if data is None:
            raise ClientError(f"Failed to extract post {post_id} from {url}")

        # Comments (including nested replies) are lazy-loaded by Facebook and are
        # NOT present in the initial prefetched GraphQL payload. Render the post
        # permalink in the real browser, expand every "View N replies" affordance,
        # then parse role="article" nodes whose aria-label starts with "Comment by"
        # or "Reply by X to Y's comment". Replies are attached to their parent via
        # the permalink href (comment_id=PARENT&reply_comment_id=SELF).
        comments = self._extract_comments_from_rendered_page(url, post_id)
    ...
    def comment_on_post(self, post_url: str, text: str) -> Dict:
        ...
        # Facebook personalizes the aria-placeholder/aria-label (e.g. "Answer as Adam",
        ...
        # Multi-stage verification (composer-cleared -> count-delta -> text-on-page).
        # Returns either confirmed or render-timeout-likely-success; raises only
        # when ALL signals fail (true submit failure). Callers see verification
        # status in the result envelope.
