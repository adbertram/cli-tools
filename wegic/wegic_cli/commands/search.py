...
app = typer.Typer(help="Search Wegic", no_args_is_help=True)
...
def search_query(
    ...
    """
    Search for items on Wegic.

    Example:
        wegic search query "my search term" --limit 20 --table
        wegic search query "keyword" --filter "price:lt:100"
        wegic search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        wegic search item abc123 --table
        wegic search item "https://example.com/item/abc123"
        wegic search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    """
    List items from Wegic.

    Example:
        wegic search list --limit 100 --table
        wegic search list --filter "status:active"
        wegic search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        wegic search wildcard "*test*"
        wegic search wildcard "prefix*"
        wegic search wildcard "*pending*" --fields "status,notes"
        wegic search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
