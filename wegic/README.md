...
A command-line interface for [Wegic](https://wegic.ai/) using browser automation. Browser automation for Wegic site workflows
...
After installation, the `wegic` command will be available in your terminal.
...
# Search for items
wegic search query "search terms"

# Get item details
wegic search item ITEM_ID
```

## Commands

### Authentication (`wegic auth`)
...
wegic auth login --profile work

# Use named profile for status check
wegic auth status --profile work

# Set default profile via environment variable
export WEGIC_DEFAULT_PROFILE=work
wegic auth status  # Uses 'work' profile

# Logout specific profile
wegic auth logout --profile work
...
### Profiles (`wegic auth profiles`)

```bash
# List all profiles
wegic auth profiles list

# Show default profile
wegic auth profiles get default

# Switch default profile
wegic auth profiles set-default PROFILE_NAME

# Create a new profile
wegic auth profiles create PROFILE_NAME
```
...
# Limit results
wegic search query "search terms" --limit 10

# Get item details
wegic search item ITEM_ID
wegic search item https://example.com/item/123
...
WEGIC_USERNAME=your_username
WEGIC_PASSWORD=your_password

# Base URL
WEGIC_BASE_URL=https://wegic.ai/

# Browser settings (true = invisible, false = visible browser)
WEGIC_HEADLESS=true

# Authentication Configuration
WEGIC_AUTH_COOKIE_NAMES=session.*,auth,token,sid  # Regex patterns for auth cookies
WEGIC_AUTH_SELECTOR=                               # CSS selector indicating authenticated state
WEGIC_AUTH_URL_PATTERN=                            # URL pattern indicating login page
WEGIC_AUTH_TIMEOUT=60                              # Seconds to wait for login
WEGIC_AUTH_POLL_INTERVAL=2                         # Seconds between auth checks
WEGIC_DEFAULT_PROFILE=default                      # Default profile name
```
...
- **Headless mode**: Set `WEGIC_HEADLESS=false` to see the browser (useful for debugging)
...
export WEGIC_HEADLESS=false
wegic search query "test"
```
