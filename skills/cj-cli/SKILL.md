...
3. Reinstalled the editable shared infra into the cj uv tool venv: `uv pip install --python ~/.local/share/uv/tools/cj-cli/bin/python3 -e <cli-tools-root>/cli-tools-shared`.
...
3. `cd cj && uv run --with pytest --with <cli-tools-root>/cli-tools-shared pytest` — 8 passed (includes new `test_bug5_cj_browser_uses_absence_of_login_form_check`).
...
1. `cd <cli-tools-root>/cj && UV_PROJECT_ENVIRONMENT=~/.cache/uv/project-envs/cj-tests uv run --with pytest --with <cli-tools-root>/cli-tools-shared pytest tests/test_bugfixes.py` — 19 passed (6 new Bug-7 tests + 13 prior bug regressions).
