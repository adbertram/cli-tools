...
### 8. Clip Brainstorming Fact Check Flag Must Override Reset
**Symptom:** `coursecraft clips update <record-id> --brainstorming-outline "<text>" --brainstorming-outline-fact-checked` can clear `Brainstorming Outline Fact Checked` when the clip was already fact-checked before the outline update.
**Cause:** Clip updates applied the explicit `--brainstorming-outline-fact-checked` field before the automatic reset that runs when a previously fact-checked brainstorming outline changes. Course and module updates already applied the explicit flag after the reset.
**Fix:** Move the clip explicit fact-check assignment after the automatic reset in `coursecraft_cli/commands/clips.py`.
**Verification:** Run `python3 -m pytest tests/test_clips_update.py -q` and `python3 -m pytest`.
**Recurrence Prevention:** For outline update commands, keep explicit user-provided fact-check flags after automatic invalidation logic so the explicit command wins.
...
**Fix:** Add `--template-deck-version` to `coursecraft_cli/commands/templates.py` create and update paths so template records can be corrected through the CLI.
