...
```bash
ls <cli-tools-root>/skills/<tool>-cli/ 2>/dev/null
```
...
```
<cli-tools-root>/skills/<tool>-cli/usage.json
```
...
Write to `<cli-tools-root>/skills/<tool>-cli/SKILL.md`.

## Step 6: Create Skill Directory

```bash
mkdir -p <cli-tools-root>/skills/<tool>-cli
```
...
```bash
<cli-tools-root>/skills/create-cli-tool-skill/scripts/validate-usage-json.sh <cli-tools-root>/skills/<tool>-cli/usage.json
```
