...
def test_is_git_repo(cli_name, cli_dir, command_filter):
    """Assertion 7a: CLI tools must live in the parent cli-tools monorepo."""
    if command_filter:
        pytest.skip("Skipping general setup tests (command filter active)")

    parent_repo = cli_dir.parent
    assert (parent_repo / ".git").exists(), (
        f"Parent cli-tools directory is not a git repository: {parent_repo}. "
        "Fix: initialize the cli-tools root as the monorepo."
    )
    assert not (cli_dir / ".git").exists(), (
        f"CLI tool '{cli_name}' still has a nested .git folder. "
        "Fix: remove nested tool repositories; only the cli-tools parent should be a git repo."
    )
...
def test_gitignore_exists(cli_name, cli_dir, command_filter):
    """Assertion 7a2: CLI tool must have a .gitignore file."""
    if command_filter:
        pytest.skip("Skipping general setup tests (command filter active)")

    gitignore = cli_dir / ".gitignore"
    assert gitignore.exists(), (
        f"CLI tool '{cli_name}' has no .gitignore file. "
        f"Fix: Create {cli_dir}/.gitignore with at minimum: .venv/, __pycache__/, *.pyc, *.egg-info/, .env, .DS_Store"
    )
    ...
    assert not missing, (
        f"CLI tool '{cli_name}' .gitignore is missing required patterns: {', '.join(missing)}. "
        f"Fix: Add these to {gitignore}"
    )


def test_github_remote_configured(cli_name, cli_dir, command_filter):
    """Assertion 7b: parent cli-tools repo must point at adbertram/cli-tools."""
    if command_filter:
        pytest.skip("Skipping general setup tests (command filter active)")
    ...
    assert result.returncode == 0, (
        "Parent cli-tools repo has no origin remote configured. "
        "Fix: gh repo create adbertram/cli-tools --private --source=. --push"
    )

    remote_url = result.stdout.strip()
    assert remote_url in {
        "git@github.com:adbertram/cli-tools.git",
        "https://github.com/adbertram/cli-tools.git",
        "https://github.com/adbertram/cli-tools",
    }, (
        f"Parent cli-tools origin remote is wrong: {remote_url}. "
        "Fix: git remote set-url origin git@github.com:adbertram/cli-tools.git"
    )


def test_github_repo_exists(cli_name, command_filter):
    """Assertion 7c: parent cli-tools GitHub repo must be private."""
    if command_filter:
        pytest.skip("Skipping general setup tests (command filter active)")
    ...
    result = subprocess.run(
        ["gh", "repo", "view", "adbertram/cli-tools", "--json", "nameWithOwner,visibility"],
        capture_output=True,
        text=True,
        timeout=30,
    )
    assert result.returncode == 0, (
        "GitHub repository adbertram/cli-tools not found. "
        "Fix: gh repo create adbertram/cli-tools --private --source=. --push"
    )

    repo_info = json.loads(result.stdout)
    assert repo_info.get("visibility") == "PRIVATE", (
        "GitHub repository 'adbertram/cli-tools' exists but is not private. "
        "Fix: gh repo edit adbertram/cli-tools --visibility private"
    )
...
def test_has_cli_tool_skill(cli_name, command_filter):
    ...
    Every CLI tool needs a skill at cli-tools/skills/<name>-cli/ with at least
