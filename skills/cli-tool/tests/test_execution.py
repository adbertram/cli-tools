...
def test_get_commands_execute_successfully(cli_executable, cli_name, test_config, cli_fixtures, command_filter, require_authenticated):
    ...
    main_help = run_cli_command(cli_executable, ["--help"], timeout=timeout).stdout
    ...
    # Find groups with both list and get
    get_groups = []
    for group in subgroups:
        group_help = run_cli_command(cli_executable, [group, "--help"], timeout=timeout).stdout
        has_list = " list " in group_help or "│ list" in group_help
        has_get = " get " in group_help or "│ get" in group_help
        if has_get and has_list:
