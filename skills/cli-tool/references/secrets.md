...
```bash
<cli-tools-root>/secret-manager/secrets.sh
```
...
```bash
<cli-tools-root>/secret-manager/secrets.sh set <name> [value]
<cli-tools-root>/secret-manager/secrets.sh get <name>
<cli-tools-root>/secret-manager/secrets.sh has <name>
<cli-tools-root>/secret-manager/secrets.sh delete <name>
<cli-tools-root>/secret-manager/secrets.sh list
```

Use stdin or `SECRET_VALUE` for secret values so they do not appear in shell history:

```bash
printf '%s' "$SECRET_VALUE" | <cli-tools-root>/secret-manager/secrets.sh set <name>
SECRET_VALUE="$SECRET_VALUE" <cli-tools-root>/secret-manager/secrets.sh set <name>
```
