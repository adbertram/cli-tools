...
<cli-tools-root>/skills/cli-tool/scripts/new-cli-tool \
  --name <name> \
  --type api \
  --base-url <url> \
...
```bash
<cli-tools-root>/skills/cli-tool/scripts/validate-cli-tool.sh <name>
```
...
```bash
<cli-tools-root>/skills/cli-tool/scripts/test-cli-tool.sh --cli-name <name>
```
...
Add the new CLI to `<cli-tools-root>/docs/cli_tools.md`:
...
This generates a repo-owned skill at `<cli-tools-root>/skills/<name>-cli/` with `SKILL.md` and `usage.json`, enabling natural language command discovery for the new CLI.
