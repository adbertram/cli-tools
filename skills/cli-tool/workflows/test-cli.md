...
```bash
<cli-tools-root>/skills/cli-tool/scripts/test-cli-tool.sh --cli-name "$TOOL_NAME" [--command "$COMMAND"] [--verbose]
```
...
4. Run test script to verify the fix:
   ```bash
   <cli-tools-root>/skills/cli-tool/scripts/test-cli-tool.sh --cli-name "$TOOL_NAME" [--command "$COMMAND"]
   ```
...
<configuration>
Test behavior is controlled by `<cli-tools-root>/skills/cli-tool/tests/cli_test_config.toml`
...
Create `<cli-tools-root>/skills/cli-tool/tests/fixtures/<cli-name>.json`:
...
Browser CLIs should be nearly identical to the `_templates/browser/` template. The template provides:
