...
A command-line interface for [Setme](https://www.setme.net/) using browser automation. Browser CLI for SetMe workflows
...
After installation, the `setme` command will be available in your terminal.
...
# Search for items
setme search query "search terms"

# Get item details
setme search item ITEM_ID
```

## Commands

### Authentication (`setme auth`)
...
setme auth login --profile work

# Use named profile for status check
setme auth status --profile work

# Set default profile via environment variable
export SETME_DEFAULT_PROFILE=work
setme auth status  # Uses 'work' profile

# Logout specific profile
setme auth logout --profile work
...
### Profiles (`setme auth profiles`)

```bash
# List all profiles
setme auth profiles list

# Show default profile
setme auth profiles get default

# Switch default profile
setme auth profiles set-default PROFILE_NAME

# Create a new profile
setme auth profiles create PROFILE_NAME
```
...
# Limit results
setme search query "search terms" --limit 10

# Get item details
setme search item ITEM_ID
setme search item https://example.com/item/123
...
SETME_USERNAME=your_username
SETME_PASSWORD=your_password

# Base URL
SETME_BASE_URL=https://www.setme.net/

# Browser settings (true = invisible, false = visible browser)
SETME_HEADLESS=true

# Authentication Configuration
SETME_AUTH_COOKIE_NAMES=session.*,auth,token,sid  # Regex patterns for auth cookies
SETME_AUTH_SELECTOR=                               # CSS selector indicating authenticated state
SETME_AUTH_URL_PATTERN=                            # URL pattern indicating login page
SETME_AUTH_TIMEOUT=60                              # Seconds to wait for login
SETME_AUTH_POLL_INTERVAL=2                         # Seconds between auth checks
SETME_DEFAULT_PROFILE=default                      # Default profile name
```
...
- **Headless mode**: Set `SETME_HEADLESS=false` to see the browser (useful for debugging)
...
export SETME_HEADLESS=false
setme search query "test"
```
