...
app = typer.Typer(help="Search Setme", no_args_is_help=True)
...
def search_query(
    ...
    """
    Search for items on Setme.

    Example:
        setme search query "my search term" --limit 20 --table
        setme search query "keyword" --filter "price:lt:100"
        setme search query "keyword" --properties "title,price"
    """
    try:
...
def search_item(
    ...
    Example:
        setme search item abc123 --table
        setme search item "https://example.com/item/abc123"
        setme search item abc123 --properties "title,price"
    """
    try:
        client = get_client()
...
def search_list(
    ...
    """
    List items from Setme.

    Example:
        setme search list --limit 100 --table
        setme search list --filter "status:active"
        setme search list --properties "id,name,status"
    """
    try:
        # Validate filters if provided
        if filter:
...
def search_wildcard(
    ...
    Examples:
        setme search wildcard "*test*"
        setme search wildcard "prefix*"
        setme search wildcard "*pending*" --fields "status,notes"
        setme search wildcard "*error*" --properties "id,name,status" --table
    """
    try:
        client = get_client()
